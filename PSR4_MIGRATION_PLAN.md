...
на один релиз, затем удалить (обновить `deleted_files.txt`, `LB_ROOT_FILES`, PHPStan
`scanFiles`); остаются только Composer + ModuleLoader; финальный `composer dump-autoload`;
коммит `vendor/`.
