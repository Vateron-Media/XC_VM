...
Интерактивный, всегда синхронизированный пользовательский интерфейс Swagger для каждого XC_VM API, созданный на основе спецификаций OpenAPI 3.0. Используйте вкладки API в верхней части страницы для переключения между API или откройте один из них напрямую по ссылкам ниже.
...
|**API для игроков**|Проигрыватель XtreamCodes — Прямой эфир, VOD, Серия, EPG|`username` + `password`|[Открыть ↗](../../_media/swagger-ui.html?spec=player)|
...
2. Разверните любую конечную точку → ** Попробуйте ** → заполните параметры → ** Выполните**, чтобы увидеть реальный URL запроса, команду cURL и оперативный ответ.
