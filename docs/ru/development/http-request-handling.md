...
Рабочий процесс администрирования не использует `Request::capture()`. Вместо этого `LegacyInitializer::initCore()` управляет обработкой входных данных:

1. `InputValidator::cleanGlobals()` вызывается на месте `$_GET`, `$_POST`, `$_SESSION`, и `$_COOKIE`, удаляя нулевые байты, последовательности обхода пути (`../`) и символы переопределения RTL.
...
Точка входа: `www/stream/*.php` конечные точки или `Public/index.php` для `player_api`
...
Процедурный защитный скрипт, включенный ранее в устаревший bootstrap. Выполняется только для HTTP-запросов (пропускается, если задано значение `$_SERVER['argc']`, указывающее CLI).
...
3. **Проверка хоста** - Когда `$rSettings['verify_host']` имеет значение true, проверяется, что `HOST` отображается в кэшированном списке `allowed_domains`. Исключения: имя хоста `xc_vm` и любой действительный IP-адрес всегда разрешены.
4. **Флаг отображения ошибки** -- Устанавливает константу `PHP_ERRORS` вместо `$rSettings['debug_show_errors']`.
...
|Метод|Описание|
| --- | --- |
| `cleanGlobals(&$data, $iteration)` |Удаление нулевых байтов на месте, обход пути (`../`) и переопределение символов RTL. Максимум 10 уровней рекурсии.|
...
| `parseCleanKey($key)` |Наносим двойные точки, узоры `__wrapped__`, применяем `htmlspecialchars(urldecode())`.|
| `parseCleanValue($value)` |Удаляет теги `<script`, HTML-комментарии, нормализует разрывы строк, обрабатывает кодировку сущностей.|

### Подтверждение действия

|Метод|Описание|
| --- | --- |
| `validate($action, $data)` |Возвращает `true`/`false` информацию о том, соответствует ли `$data` минимальным требованиям для данного действия API.|
...
`LegacyInitializer::initCore()` звонков:
...
Объектно-ориентированная оболочка запроса. Содержит статическую фабрику `capture()` и методы экземпляра для доступа к обработанным входным данным. Хотя класс существует и полностью функционален, в основном производственном потоке вместо него используется `InputValidator` + `RequestManager`. Методы статической очистки класса `Request` (`cleanGlobals`, `parseIncomingRecursively`) используются `LegacyInitializer::initStreaming()` для обеспечения обратной совместимости.
...
| `post` | `post($key = null, $default = null)` |Получаем данные из POST. `null` key возвращает все.|
...
| `server` | `server($key, $default = null)` |Получаем `$_SERVER` значение|
...
| `rawBody` | `rawBody()` |Необработанный текст сообщения от `php://input`|
...
Эта нормализация применяется как во время регистрации (`buildRoute`), так и во время отправки (`normalizePage`), поэтому маршруты, зарегистрированные как `watch/add`, соответствуют названиям страниц, таким как `watch_add`.
...
1. Нормализовать `$page` (символы подчеркивания заменить косыми чертами, убрать `.php`).
...
3. **Проверка прав доступа ** через `checkPermission()`. Если отказано, наберите `denyAccess()` (переадресация или 403).
4. **Выполнение промежуточного программного обеспечения**. Вызывается каждый вызываемый объект в массиве `middleware`. Если какой-либо из них возвращает `false`, выполнение останавливается.
...
Важно: `dispatchApi()` не запускает промежуточное программное обеспечение. Это намеренное отличие от отправки страницы.

### Регистрация маршрута модуля

Модули регистрируют маршруты через `ModuleInterface::registerRoutes()`. Маршрутизатор поддерживает безопасный режим регистрации, предотвращающий перезапись модулями основных маршрутов:
...
| `getRoutes()` |Возвращает все зарегистрированные ключи маршрута как `['get' => [...], 'post' => [...], 'api' => [...]]`|
...
| `noCache` | `noCache()` |Установить заголовки без кэширования (используется для плейлистов HLS)|
...
| `CONTEXT_STREAM` |+ Только база данных (упрощенная, без `LegacyInitializer`). Конечные точки потоковой передачи используют `StreamingRequestBootstrap` вместо этого.|
| `CONTEXT_ADMIN` |+ Сессия + База данных + `LegacyInitializer::initCore()` + Redis + Admin API + Переводчик + глобальные настройки администратора. Полная инициализация.|
...
| `src/bootstrap.php` |Унифицированный bootstrap (`XC_Bootstrap` class)|
