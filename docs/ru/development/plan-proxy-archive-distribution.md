...
- [x] Исправить `\ ` → `\;` во всех `find -exec` (`set_permissions`) — сделано (f67176e).
- [x] Убрать маскировку ошибок `|| [ $$? -eq 1 ]` — сделано: baseline 0755/0644,
      затем точечные `2>/dev/null || true` только на опциональных путях (f67176e).
...
- [x] Генерировать `hashes.md5` + `hashes.sha256` — сделано (`hashes` target,
      формат `<hash>␠␠proxy.tar.gz`, парсится `getAssetHash`; проверено live).
- [x] **Детерминированный tar** — сделано (f67176e); `check-reproducible` зелёный. (иначе «побайтно эквивалентный» архив в этапе 1
...
- [x] `version.json` (git tag + sha, без wall-clock) — сделано. `git describe --tags` — после появления тегов
...
- [x] Workflow `.github/workflows/build-release.yml` — сделано (f67176e):
      триггеры `release: [created]` + `workflow_dispatch(tag)`
...
- [x] Схема версионирования: **bare semver-теги `X.Y.Z` без `v`-префикса** (workflow guard) —
...
- [x] CI: публиковать релиз как **draft**, залить `proxy.tar.gz`+хэши, и только
...
- [x] Smoke воспроизводимости (`check-reproducible`) — сделано: прогнать `make` дважды, сверить
...
- [x] Первый релиз `1.0.0` — **опубликован и проверен live** (`cron:proxy` скачал,
      MD5 сошёлся, индекс записан). Цель — **content-эквивалентность** текущему
