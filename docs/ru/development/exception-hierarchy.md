...
Все исключения XC_VM расширяют `XcVmException`, так что вызывающие абоненты могут перехватывать все дерево с помощью
...
| `NotFoundException` |`get($id)` запросил незарегистрированную услугу|
