...
2. `_xc_nav_label()` - разрешение текстовой надписи.
...
Проверки выполняются в режиме `_xc_nav_visible()`:

1. `desktopOnly`: скрывает узел на мобильном устройстве.
2. `settingDisabled`: скрывает узел, когда включен флаг настройки.
3. `permissions`: ИЛИ-проверить через `Authorization::check('adv', $permission)`.
...
2. Установите `parent` для существующего основного узла дерева или вашего собственного, уже добавленного узла.
