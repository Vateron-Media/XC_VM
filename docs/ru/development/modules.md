...
> производный от `name`/`version`). Проставляется командой `make module-hashes`
> (`php tools/gen-module-hashes.php`) после создания нового модуля — идемпотентно, у кого id уже
> есть, не трогает. Руками не писать. Даёт стабильную идентичность независимо от `name` — основа
...
- [ ] Прогнать `make module-hashes` — проставит постоянный `hash_id` (идемпотентно; руками не писать)
