...
Контекст выражается в виде значения `BootContext` enum.
...
Не включает: базу данных, Redis, сеансы, переводчик, API-интерфейсы администратора.
...
Добавляет инициализацию базы данных и устаревшего ядра через `MINIMAL`.

Включает в себя:

- Подключение к базе данных
- `LegacyInitializer`
- необязательный Redis (`'redis' => true`)
...
Не включает: Redis, переводчик, API-интерфейсы администратора, сеансы.
...
| `redis` | `bool` |`true` для АДМИНИСТРАТОРА, `false` в противном случае|Подключение Redis|
