...
> автоматически (установщик запускает `openssl` и записывает `server.key`/`server.crt`
...
После выполнения всех шагов в @@0 должны присутствовать следующие файлы@@:
...
Ваш сервер **XC_VM Nginx** теперь доступен по протоколу **HTTPS** с использованием недавно созданного самозаверяющего сертификата.
