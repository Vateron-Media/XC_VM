...
- Данные файловой системы (записи, VOD files, EPG XML)
...
1. Закройте MySQL соединение перед сбросом.
2. Запустите `mysqldump --no-data` (структура) + `mysqldump --ignore-table` (данные, за исключением таблиц журнала).
...
1. Если существует локальный файл, используйте его. В противном случае загрузите из Dropbox на `/home/xc_vm/tmp/restore.sql`.
...
- Если `dropbox_keep > 0`: в Dropbox сохраняются только N самых последних файлов. Самые старые удаляются первыми.
...
Когда `dropbox_remote` включен:

1. После создания локальной резервной копии загрузите ее в Dropbox.
2. Во время загрузки создается файл `.uploading` marker.
3. В случае успеха: `.uploading` удаляется.
4. При сбое: `.error` создается файл с сообщением об ошибке.
