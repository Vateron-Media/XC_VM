...
- **`/api` отвечает, но heartbeat устарел** — нода достучивается до main, но её `cron:servers` не работает или не может писать в БД панели.
...
5. **Крон отчётности** — есть ли `cron:servers` в crontab (он обновляет heartbeat, за которым следит main; если отсутствует — перезапустите `console.php startup`).
6. **Расхождение часов** — `time_offset` относительно панели.
...
| `/api` в порядке, heartbeat устарел | Нет `cron:servers` или ошибка записи в БД | `console.php startup` на ноде; проверить гранты БД (`tools mysql` на main) |
| Нода «мигает» онлайн/офлайн | Расхождение часов > 30 с | Синхронизировать NTP на ноде |
...
| `src/Cli/CronJobs/ServersCronJob.php` | Heartbeat, о котором рассуждает команда |
