...
|**Все узлы падают в один и тот же момент**|Перезапуск MySQL /сбой на главном сервере приводит к одновременному сбою watchdog на каждом узле (фатально для сборок до исправления; текущие сборки переждут это).|Проверьте журнал ошибок MySQL на главном сервере во время сброса; обновите узлы, чтобы watchdog пережил перебои в работе|
