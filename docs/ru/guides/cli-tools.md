...
Справочник по интерфейсу командной строки XC_VM, системным инструментам и процессу обновления базы данных после обновления версии. Охватывает ежедневные операции, экстренный доступ и создание новых этапов обновления базы данных.
...
|**Демоны**|8|Длительно выполняющиеся фоновые процессы (команды с использованием `DaemonTrait`)|
...
| `migrate` | `MigrateCommand` |Передача данных из базы данных `xc_vm_migrate`|xc_vm|
...
| `server:diagnose` | `ServerDiagnoseCommand` |Диагностируйте, почему прокси-узел/LB-узел не подключен к основному (сердцебиение, доступность, iptables, сервис).|корень|
...
| `loopback` | `LoopbackCommand` |Получать MPEG-TS с другого сервера|
...
Все имена заданий cron имеют префикс `cron:`. Они используют `CronTrait` и вызываются системной crontab.
...
| `cron:maxmind` | `MaxMindCronJob` |Обновление баз данных MaxMind GeoIP (только по вторникам; `--force` запускать вручную)|
...
| `cron:users` | `UsersCronJob` |Управление подключениями пользователей, Redis синхронизация, расхождение|
| `cron:vod` | `VodCronJob` |Обрабатывать VOD содержимое|
...
> Необязательные задания cron (зарегистрированные условно): `cron:backups`, `cron:cache_engine`, `cron:epg`, `cron:providers`, `cron:root_mysql`, `cron:series`, `cron:tmdb`, `cron:tmdb_popular`, `cron:update`.

---

## Регистрация новой команды

Все команды CLI реализуют `CommandInterface`. Основные команды автоматически обнаруживаются из `src/Cli/` через отражение в `console.php`. Команды модуля регистрируются через `ModuleLoader::registerAllCommands()`.
...
Если команда не должна включаться в сборки балансировщика нагрузки, добавьте ее путь к `LB_FILES_TO_REMOVE` в поле `Makefile`.
...
| `database` |Восстановите пустую базу данных XC_VM из `database.sql`. **Удаляет ВСЕ данные!** Требуется флаг `--confirm`.|
...
| `duplicates` |Найдите и удалите повторяющиеся потоки VOD. Группируйте по идентичному источнику, сначала сохраняйте, остальные удаляйте. **Разрушительный!**|
...
- ❗️ ** Внимание: ** `duplicates` безвозвратно удаляет трансляции и все связанные с ними данные (журналы, статистику, эпизоды, записи). Всегда создавайте резервную копию перед запуском.
...
- Применяемые шаги обновления базы данных отслеживаются в таблице `migrations` базы данных. Каждый шаг выполняется **ровно один раз** - если шаг уже был применен, он пропускается.

- Обновления базы данных выполняются автоматически:
  - `console.php update post-update` - после обновления панели
  - `console.php status` - во время проверки состояния системы (только на ГЛАВНОМ сервере)
...
- Префикс номера: 3 цифры, дополненные нулем (например, `006`, `007`)
...
Поместите в файл необработанные инструкции SQL. Несколько инструкций разделяются символами `;`.
...
> 💡 В хранилище исходных текстов это `src/migrations/`.
...
Если инструкция не выполняется, шаг все равно будет записан, но покажет `[WARN]` — просмотрите SQL и устраните любые проблемы.
...
Применяет все ожидающие обновления файлы `.sql` из `/home/xc_vm/migrations/`. Используйте это, когда вам нужно выполнить миграцию без полного обновления системы.
