...
XC_VM использует двухуровневую защиту для входящих данных запроса. Во-первых, программа глобальной очистки удаляет опасный контент со всех PHP суперглобальных ресурсов во время начальной загрузки, перед запуском любого кода приложения. Во-вторых, уровень проверки на уровне действия проверяет наличие обязательных полей перед выполнением бизнес-логики.

Оба уровня реализованы в `src/Core/Validation/InputValidator.php`.
...
Контекст потоковой передачи (`LegacyInitializer::initStreaming()`) выполняет ту же последовательность очистки, используя класс `Request`, который предоставляет эквивалентные методы для пути начальной загрузки потоковой передачи.
...
3. Удаляет маркеры стиля `__dunder__` с помощью регулярного выражения
...
|Шаг|Что он делает|
| --- | --- |
|Неэкранированный пейзаж|`stripslashes()` и восстановите `&#032;` в исходное положение|
...
Проверяет наличие минимально необходимых полей для данного действия. Возвращает значение `true`, если данные приемлемы, и `false`, если необходимые поля отсутствуют или искажены. Контроллеры должны вызывать это значение перед отправкой данных на уровни службы/хранилища.
...
Фильтрует массив, чтобы он содержал только целые положительные идентификаторы. Любое значение, в котором `intval($id) <= 0` не используется. Широко используется в кодовой базе (более 30 сайтов для звонков) везде, где необходимо очистить списки идентификаторов, предоставленные пользователем, перед запросами к базе данных.
...
| --- | --- | --- |
| `processStream` |`stream_display_name` ИЛИ `review` флаг ИЛИ `$_FILES['m3u_file']`|Любой из трех параметров удовлетворяет требованиям проверки|
| `processChannel` |`stream_display_name` ИЛИ `review` флаг ИЛИ `$_FILES['m3u_file']`|Те же правила, что и в processStream|
| `processRadio` |`stream_display_name` ИЛИ `review` флаг ИЛИ `$_FILES['m3u_file']`|Те же правила, что и в processStream|

#### Фильмы / VOD

|Действие|Обязательные для заполнения поля|Записи|
| --- | --- | --- |
| `processMovie` |`stream_display_name` ИЛИ `review` флаг ИЛИ `$_FILES['m3u_file']`|Те же правила, что и в processStream|
...
**Неявно передаваемые действия** (вообще не в переключателе, перехватываются по умолчанию `return true`):

Любая строка действия, не соответствующая `case`, также вернет `true`. Если для нового действия требуется стробирование ввода, необходимо явно добавить регистр.
