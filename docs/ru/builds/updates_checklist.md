# XC_VM Контрольный список подготовки к выпуску
...
> ❗️ требуется `mkdir -p dist`: `dist/` не существует в новом клоне, и `make new` стирает его — без этого перенаправление завершается ошибкой с сообщением "Нет такого файла или каталога".

**Обновление `changelog.json`** в корневом каталоге репозитория — этот файл содержит только изменения для предстоящего выпуска:
...
Сначала завершите всю работу с функциями / исправлениями / документами и убедитесь, что она уже находится в `main`.
...
> В противном случае `dist/changes.md` будет включать дополнительные фиксации релиза и принудительно вносить дополнительные правки.

### Восстановление переведенной документации

Документация написана только на **английском языке** (`docs/en`). Русское дерево
...
- `make docs-translate` повторно переводит только те файлы на английском языке, содержимое которых
изменен (для каждого файлового кэша), так что при постепенном выпуске это происходит быстро.
- **Просмотрите и зафиксируйте восстановленный `docs/ru`** — он включен в единый
снимите фиксацию (шаг 5). Никогда не редактируйте `docs/ru` вручную.
...
Это запускает `git diff` между `LAST_TAG` и `HEAD`@, извлекает удаленные файлы под `src/`, удаляет префикс `src/` и записывает результат в `src/migrations/deleted_files.txt`.

Если `LAST_TAG` не может быть автоматически обнаружен (нет сети / нет выпусков), передайте его явно:
...
После проверки, `make main` / `make lb` файл будет помещен в архив с помощью `delete_files_list` / `lb_delete_files_list`.
...
После построения `dist/` должен содержать:
...
> Скрипт обновления (`src/update`) отфильтровывает двоичные каталоги/каталоги конфигурации во время выполнения, используя жестко заданный список `UPDATE_EXCLUDE_DIRS` внутри самого скрипта Python.
...
**Тестовая установка Docker** (смотрите `tools/test-install/`) — только после сборки, так как для этого требуется `dist/XC_VM.zip`:

```bash
bash tools/test-install/test_release.sh
```

При этом создается образ, контейнер запускается с помощью systemd и программа установки запускается автоматически.
`dist/XC_VM.zip` монтируется в контейнер как том, доступный только для чтения.

> ✅ Убедитесь, что панель загружается с `http://localhost:8880` и логин администратора работает.
...
- [ ] Запустите `md5sum -c hashes.md5` для загруженных файлов
...
**Проверка качества ** — сначала запустите `make dev-tools`, после завершения - `make dev-clean`:

|Команда|Цель|
| --- | --- |
| `make dev-tools` |Установите инструменты для разработки (PHPStan, PHP-CS-Fixer) через `composer install`|
...
| `make cs` |Проверка стиля кода - импорт/гигиена пространства имен (PHP—CS-Fixer, пробный запуск)|
| `make cs-fix` |Примените исправления в стиле кода на месте|
| `make gates` |PSR-4 регрессионные шлюзы (для процедурного использования, LB-архив, только для продукта поставщика)|
| `make dev-clean` |Снова удалите инструменты разработки, восстановив рабочий режим только для `vendor/`|
...
| `make new` |Сбросьте выходной каталог `dist/` (запустите перед сборкой)|
| `make lb` |Создайте архив LoadBalancer в `dist/`|
| `make main` |Создайте ОСНОВНОЙ архив в `dist/`|
| `bash tools/test-install/test_release.sh` |Установочный тест Docker для встроенного выпуска|

**Документация** (англоязычный источник в `docs/en`; `docs/ru` сгенерирован + зафиксирован):
...
| `make docs-build` |Строгая MkDocs сборка в `./site` (что запускает CI)|
| `make docs-serve` |Предварительный просмотр документов в режиме реального времени на странице `http://127.0.0.1:8000`|
