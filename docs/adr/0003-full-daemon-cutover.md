...
- **⚠️ TODO — LLOD v3 (`LlodCommand`) has NO daemon path.** On-demand routing (`MonitorCommand`:329-342 by `streams.llod`): llod=0 and llod=1 ("v2") go through `startStream`→`buildLive`, so A2's tee covers them; **llod=2 ("v3") goes to `startLLOD`→`LlodCommand`, a separate PHP segmenter (not ffmpeg), which A2 does not touch** — those streams stay 100% legacy (verified working, no regression). For the full cutover LLOD v3 needs its own daemon integration: either have `LlodCommand` push its mpegts into the daemon ingest socket (like the tee), or migrate v3 onto the ffmpeg/`buildLive` path. **Revisit before Phase D.**
