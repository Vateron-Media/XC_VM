...
**Remaining:** the install is now productionised in code — daemon released (0.8.0 latest), LB build ships the nginx fanout locations (`lb_configs/nginx.conf` fixed) + libogg (getPackages) + the daemon self-install (service/fanout_binary/RootSignals). **The one open item is a real-source stream-to-LB e2e** (clean source reachable from the LB IP, or restream-from-origin, plus a `streams_servers` assignment to the LB) — needs a working source (the earlier canary attempt failed on a geo/source defect, not the daemon).
