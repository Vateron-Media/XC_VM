...
**Remaining:** the install + restream paths are now productionised in code — daemon released (0.8.0 latest), LB build ships the nginx fanout locations (`lb_configs/nginx.conf` fixed) + libogg (getPackages) + the daemon self-install (service/fanout_binary/RootSignals), and loopback restream feeds the daemon. **The one open item is a real-source stream-to-LB e2e on a live node** (assign a working MAIN stream to an LB with `parent_id` + `php_loopback`, confirm the LB daemon serves it) — currently blocked: the canary LB `31.77.173.176` is unreachable by SSH (banner-exchange timeout — offline / moved / fail2ban), and the earlier attempt failed on a geo/source defect, not the daemon.
