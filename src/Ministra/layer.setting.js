...
SettingLayer.prototype.shift = function (dir) {
	...
	if (
		this.controls[this.cur_control_idx] instanceof OptionInput &&
		this.controls[this.cur_control_idx].suggests_list &&
		!this.controls[this.cur_control_idx].suggests_list.isHidden()
	) {
...
SettingLayer.prototype.bind = function () {
	...
	(function () {
		if (
			this.controls[this.cur_control_idx] instanceof OptionInput &&
			this.controls[this.cur_control_idx].suggest_input_dom_obj &&
			!this.controls[this.cur_control_idx].suggest_input_dom_obj.isHidden()
		) {
	...
	(function () {
		if (this.left_ear) {
			...
			this.controls &&
			(this.controls[this.cur_control_idx] instanceof OptionInput ||
				this.controls[this.cur_control_idx] instanceof VisualValuePickerInput)
		) {
	...
	(function () {
		if (this.controls && this.controls[this.cur_control_idx] instanceof OptionInput) {
			this.controls[this.cur_control_idx].shift_page.call(
				this.controls[this.cur_control_idx],
				1
			);
		}
