...
(function () {
	function AccountConstructor() {
		...
		this.init = function () {
			...
			this.tab["main"] = new Showable(create_block_element("main_container", this.container))
				.setParent(this)
				.setDependencies(this.tab, ["payment", "agreement", "terms"]);
			this.tab["main"].content = new Scrollable(
				create_block_element("main_content", this.tab["main"].dom_obj),
				this.tab["main"].dom_obj
			);
			...
			stb.load(
				...
				function (result) {
					...
					if (this.account_data["account_payment_info"]) {
						this.tab["payment"].content.dom_obj.innerHTML =
							this.account_data["account_payment_info"];
					}

					if (this.account_data["account_agreement_info"]) {
						this.tab["agreement"].content.dom_obj.innerHTML =
							this.account_data["account_agreement_info"];
					}

					if (this.account_data["account_terms_info"]) {
						this.tab["terms"].content.dom_obj.innerHTML =
							this.account_data["account_terms_info"];
					}
				},
				this
			);
			...
			this.tab["payment"] = new Showable(
				create_block_element("payment_container", this.container)
			)
				.setParent(this)
				.setDependencies(this.tab, ["main", "agreement", "terms"]);
			this.tab["payment"].content = new Scrollable(
				create_block_element("payment_content", this.tab["payment"].dom_obj),
				this.tab["payment"].dom_obj
			);
			...
			this.tab["agreement"] = new Showable(
				create_block_element("agreement_container", this.container)
			)
				.setParent(this)
				.setDependencies(this.tab, ["main", "payment", "terms"]);
			this.tab["agreement"].content = new Scrollable(
				create_block_element("agreement_content", this.tab["agreement"].dom_obj),
				this.tab["agreement"].dom_obj
			);
			...
			this.tab["terms"] = new Showable(
				create_block_element("terms_container", this.container)
			)
				.setParent(this)
				.setDependencies(this.tab, ["main", "payment", "agreement"]);
			this.tab["terms"].content = new Scrollable(
				create_block_element("terms_content", this.tab["terms"].dom_obj),
				this.tab["terms"].dom_obj
			);
		...
		this.shift_page = function (dir) {
			...
			this.tab[this.cur_tab].content.scrollPage &&
				this.tab[this.cur_tab].content.scrollPage(dir);
		};
		...
		this.bind = function () {
			...
			(function () {
				...
				this.get_return_layer().show(
					stb.user.hasOwnProperty("block_msg") ? stb.user["block_msg"] : ""
				);
			})
				.bind(key.MENU, this)
				.bind(key.EXIT, this)
	...
	module.account_menu.push({
		...
		cmd: function () {
			_debug(
				"stb.profile[account_page_by_password]",
				stb.profile["account_page_by_password"]
			);
