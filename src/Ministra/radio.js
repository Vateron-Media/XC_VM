...
(function () {
	...
	function radio_constructor() {
		...
		this.play = function () {
			...
			this.update_header_path([
				{ alias: "playing", item: this.data_items[this.cur_row].name },
			]);
		...
		this.set_active_row = function (num) {
			...
			if (
				...
				stb.player.cur_media_item.id == this.data_items[this.cur_row].id
			) {
				...
				typeof stb.player.radio_idx != "undefined" ||
				(stb.player.cur_media_item && stb.player.cur_media_item.radio)
			) {
				var idx =
					typeof stb.player.radio_idx != "undefined"
						? stb.player.radio_idx
						: this.data_items.getIdxByVal("id", stb.player.cur_media_item.id);
				idx = idx || 0;
				/*this.superclass.set_active_row.call(this, idx);*/
				if (
					stb.player.cur_media_item &&
					this.data_items[idx] &&
					this.data_items[idx].id == stb.player.cur_media_item.id
				) {
		...
		this.del_from_fav = function () {
			...
			var fav_radio_idx = stb.player.fav_radio.getIdxByVal(
				"id",
		...
		this.load_fav_radio = function () {
			stb.load(
				...
				function (result) {
					...
					stb.player.fav_radio =
						typeof result != "undefined" && result ? result.data : [];
	...
	radio.init_sort_menu(sort_menu_map, {
		offset_x: 217,
		color: "green",
	});
	...
	radio.init_fav_menu(fav_menu_map, {
		color: "yellow",
	...
	radio.init_search_menu(search_menu_map, {
		color: "blue",
		need_update_header: false,
	});
