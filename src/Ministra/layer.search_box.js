...

		{
			...
			map: [
				...
				},
				{
					...
					cmd: (function (self) {
						if (
							self.languages.indexOf("ru") >= 0 ||
							self.languages.indexOf("uk") >= 0
						) {
							return function () {
								self.change_lang();
							};
...
search_box.prototype.show = function () {
	...
	this.dom_obj.show();
	this.dom_obj.moveY(
		this.parent.dom_obj.offsetHeight -
			this.dom_obj.clientHeight -
			this.parent.color_buttons.buttons_bar.offsetHeight
	);
	if (this.color) {
		...
		var offset_left =
			button.offsetLeft +
			this.parent.color_buttons.table.offsetLeft +
			button.offsetWidth / 2 -
			this.dom_obj.offsetWidth / 2 +
			7;
...
search_box.prototype.del = function () {
	...
	this.search_input.innerHTML = this.search_input.innerHTML.substring(
		0,
		this.search_input.innerHTML.length - 1
	);
};
...
search_box.prototype.action = function (type) {
	...
	if (type == "del") {
		...
		this.layers[this.cur_layer_idx].items[this.cur_row][this.cur_cell].cmd &&
	...
	try {
		if (
			this.parent.on &&
			(this.search_input.innerHTML.length >= 3 || this.search_input.innerHTML.length == 0)
		) {
			this.parent.update_header_path([
				{ alias: "search", item: this.search_input.innerHTML },
			]);
...
search_box.prototype.set_active_cell = function () {
	...
	this.layers[this.cur_layer_idx] &&
		...
		this.layers[this.cur_layer_idx].items[this.cur_row][this.cur_cell].dom_obj.setClass(
			"search_active_cell"
		);
};
