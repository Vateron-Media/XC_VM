...
(function () {
	function audioclub_constructor() {
		...
		this.init_modal_windows = function () {
			...
			this.new_playlist_dialog.addItem(
				new ModalFormInput({
			...
			this.new_playlist_dialog.addItem(
				new ModalFormButton({
					...
					onclick: function () {
						var name = scope.new_playlist_dialog
							.getItemByName("playlist_name")
							.getValue();
			...
			this.select_playlist_dialog = new ModalForm({
				title: get_word("audioclub_select_playlist"),
			});

			this.select_playlist_dialog.enableOnExitClose();

			this.select_playlist_dialog.addItem(
				new ModalFormSelect({
			...
			this.select_playlist_dialog.addItem(
				new ModalFormButton({
					value: get_word("ok_btn"),
					onclick: function () {
						var playlist_id = scope.select_playlist_dialog
							.getItemByName("playlist_id")
							.getValue();

						if (playlist_id == -1) {
							scope.new_playlist_dialog.track_id =
								scope.select_playlist_dialog.track_id;
			...
			this.remove_from_playlist_confirm.addItem(
				new ModalFormButton({
					value: get_word("cancel_btn"),
					onclick: function () {
			...
			this.delete_playlist_confirm.addItem(
				new ModalFormButton({
					value: get_word("cancel_btn"),
					onclick: function () {
			...
			this.new_track_search_dialog.addItem(
				new ModalFormInput({
					name: "track_search",
					onchange: function () {
			...
			this.new_album_search_dialog.addItem(
				new ModalFormInput({
					name: "album_search",
					onchange: function () {
						if (
		...
		this.action = function () {
			...
			if (
				this.data_items[this.cur_row].is_album &&
				!this.data_items[this.cur_row].is_search_result
			) {
				this.history.push({
					page: this.cur_page,
					row: this.cur_row,
		...
		this.set_active_row = function (num) {
			...
			if (
				...
				stb.player.cur_media_item.is_track &&
				this.data_items[this.cur_row].is_track &&
			...
			if (
				this.data_items[this.cur_row].is_track ||
				this.data_items[this.cur_row].is_playlist
			) {
			...
			if (this.data_items[this.cur_row].is_playlist) {
				this.color_buttons.get("yellow").text_obj.innerHTML = get_word(
					"audioclub_remove_playlist"
				);
			} else if (this.load_params["playlist_id"]) {
				this.color_buttons.get("yellow").text_obj.innerHTML =
					get_word("remove_from_playlist");
			} else {
		...
		this.fill_short_info = function (item) {
			...
			if (item.album_name) {
				info +=
					"<span>" + get_word("audioclub_album") + ": </span>" + item.album_name + "<br>";
			}

			if (item.album_year) {
				info +=
					"<span>" + get_word("audioclub_year") + ": </span>" + item.album_year + "<br>";
			}

			if (item.album_country) {
				info +=
					"<span>" +
					get_word("audioclub_country") +
					": </span>" +
					item.album_country +
					"<br>";
			}
			...
			if (item.tracks) {
				info +=
					"<span>" + get_word("audioclub_tracks") + ": </span>" + item.tracks + "<br>";
			}
		...
		this.track_search = function () {
			...
			if (search_str) {
				...
				this.update_header_path([
					{ alias: "search_track", item: get_word("track_search") },
				]);
		...
		this.album_search = function () {
			...
			if (search_str) {
				...
				this.update_header_path([
					{ alias: "album_search", item: get_word("album_search") },
				]);
