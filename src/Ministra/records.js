...
(function () {
	...
	function Records() {
		...
		this.init = function () {
			...
			create_inline_element("rest_length_title", this.rest_length_block).innerHTML =
				get_word("rest_length_title") + ": ";
				...
				new ModalFormButton({
					...
					onclick: function () {
						...
						if (scope.delete_comfirm.record.local == 1) {
							module.pvr_local.del(
								scope.delete_comfirm.record.id,
								scope.delete_comfirm.record.file,
								function () {
									scope.load_data();
								}
							);
		...
		this.play = function (play_url) {
			...
			var channel =
				stb.player.channels[
					stb.player.channels.getIdxById(this.data_items[this.cur_row].ch_id)
				];
		...
		this.fill_list = function (data) {
			...
			this.rest_length.innerText = stb.clock.convert_sec_to_human_hours(
				this.result.records_rest_length
			);
