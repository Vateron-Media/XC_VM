		...
		stbEvent.onPortalEvent = function (params) {
			...
			if (params.hasOwnProperty("type") && params.type == "settings") {
				...
				if (
					...
					params.hasOwnProperty("subtitle_size") &&
					params.hasOwnProperty("subtitle_color")
				) {
					stb.player.set_subtitle_langs(
						stb.user.pri_subtitle_lang,
						stb.user.sec_subtitle_lang,
						stb.user.subtitle_size,
						stb.user.subtitle_color
					);
				}
				...
				if (params.hasOwnProperty("show_after_loading")) {
					stb.user.display_menu_after_loading = stb.profile.display_menu_after_loading =
						params.show_after_loading == "main_menu";
				}
...
player.prototype.set_cas = function (profile) {
	...
	try {
		...
		if (!profile["cas_ini_file"] && profile["cas_web_params"]) {
			_debug(
				"stb.SetupWebCAS",
				profile["cas_web_params"].server_addr,
				profile["cas_web_params"].company_name
			);
			stb.SetupWebCAS(
				profile["cas_web_params"].server_addr,
				profile["cas_web_params"].company_name
...
player.prototype.init_pvr_dialogs = function () {
	...
	this.local_pvr_confirm.addItem(
		new ModalFormDateTimeSelect({
			...
			onset: function (value) {
				...
				var duration =
					parseInt(duration_parts[0], 10) * 3600 + parseInt(duration_parts[1], 10) * 60;
	...
	this.local_pvr_confirm.addItem(
		new ModalFormSelect({ label: get_word("usb_device") + ":", name: "usb_device" })
	);
	this.local_pvr_confirm.addItem(
		new ModalFormInput({ label: get_word("file_name") + ":", name: "file_name" })
	);
	...
	this.local_pvr_confirm.addCustomEventListener("show", function () {
		...
		if (scope.local_pvr_confirm.deferred) {
			...
			scope.local_pvr_confirm
				.getItemByName("file_name")
				.setValue(module.pvr_local.get_record_filename(scope.local_pvr_confirm.channel));
		}
	});
};
			...
			try {
				...
				if (this.media_type == "stream" && (this.is_tv || stb.cur_place == "radio")) {
					...
					if (
						...
						this.cur_media_item.media_type != "advert" &&
				...
				try {
					if (metadata) {
						...
						if (metadata.hasOwnProperty("titles") && metadata.titles.length > 1) {
							...
							this.cur_media_item.playlist = metadata.titles.map(
								function (title, idx) {
		...
		case 4: {
			...
			if (this.cur_media_item.hasOwnProperty("ad_tracking")) {
				...
				if (this.cur_media_item.ad_tracking.hasOwnProperty("creativeView")) {
					stb.advert.track(
						this.cur_media_item.ad_tracking["creativeView"],
						"creativeView"
					);
				}
			...
			if (this.active_local_time_shift && module.time_shift_local) {
				...
				this.emulated_media_len_stop = window.setTimeout(
					function () {
						_debug("fire stop");
				...
				if (
					...
					if (this.cur_media_item.ad_must_watch.indexOf("s") > 0) {
						// time in seconds
						var lock_time = parseInt(this.cur_media_item.ad_must_watch, 10);
						lock_time =
							this.cur_media_length < lock_time ? this.cur_media_length : lock_time;
					} else {
						lock_time =
			...
			if (stb.cur_place == "vclub") {
				...
				this.send_played_video_timer = window.setTimeout(
					function () {
						self.send_played_video(
							self.cur_media_item.video_id || self.cur_media_item.id
						);
					},
		...
		case 5: {
			...
			if (this.media_type == "stream") {
				...
				if (
					...
					var series_idx = this.cur_media_item.series.lastIndexOf(
						this.cur_media_item.cur_series
					);
		...
		case 33: {
			...
			if (
				[
					...
					"MAG528",
					"MAG540",
				].indexOf(stb.type.toUpperCase()) != -1
			) {
				multiplier = 100;
			...
			if (stb.profile["hdmi_event_reaction"] >= 1 && !module.blocking.on) {
				var hdmi_reaction_timeout =
					(stb.profile["hdmi_event_reaction"] == 1 || !stb.hdmi_on
						? 5
						: stb.profile["hdmi_event_reaction"]) * multiplier;
...
player.prototype.play_last = function (dont_show_osd) {
	...
	module.tv.password_input &&
		module.tv.password_input.on &&
		module.tv.password_input.hide &&
		module.tv.password_input.hide();
	module.tv.parent_password_promt &&
		module.tv.parent_password_promt.on &&
		module.tv.parent_password_promt.hide &&
		module.tv.parent_password_promt.hide();
	...
	if (
		!play_tv_item &&
		this.cur_tv_item.lock != "1" &&
		this.cur_tv_item.censored != "1" &&
		item_genre &&
...
player.prototype.init_first_channel = function () {
	...
	if (
		...
		var var_channel_list =
				stb.user.fav_itv_on && this.fav_channels.length ? "fav_channels" : "channels",
			var_channel_idx =
				stb.user.fav_itv_on && this.fav_channels.length ? "f_ch_idx" : "ch_idx",
			channel = {};
		...
		if (
			(channel && (channel.lock == "1" || channel.censored == "1")) ||
			(channel_genre && channel_genre.censored == 1)
		) {
			channel = {};
			for (var i in this[var_channel_list]) {
				channel_genre_key = module.tv.genres.getIdxByVal(
					"id",
					this[var_channel_list][i].tv_genre_id
				);
				if (
					this[var_channel_list][i].lock != "1" &&
					this[var_channel_list][i].censored != "1" &&
					module.tv.genres[channel_genre_key] &&
					module.tv.genres[channel_genre_key].censored != 1
				) {
...
player.prototype.play = function (item) {
	...
	if (
		...
		!stb.player.cur_media_item.hasOwnProperty("is_file")
	) {
		if (item.hasOwnProperty("open") && !item.open) {
			...
			if (
				...
				(parseInt(item.use_http_tmp_link) == 1 ||
					...
					(stb.user["force_ch_link_check"] && this.is_tv))
			) {
				var self = this;
				stb.player.on_create_link = function (result) {
					...
					if (result.cmd && result.cmd.search(/%mac%/gi) !== -1) {
						result.cmd = result.cmd.replace(/%mac%/gi, stb.profile.mac);
					}
				...
				this.create_link(
					...
					0,
					false,
					item.disable_ad || (stb.advert && stb.advert.disabled),
					false,
					stb.user["force_ch_link_check"]
				);
			} else {
				if (cmd && cmd.search(/%mac%/gi) !== -1) {
					cmd = cmd.replace(/%mac%/gi, stb.profile.mac);
				}
				this.play_now(cmd);
			}
		...
		stb.player.cur_media_item.media_type == "advert" ||
...
player.prototype.play_now = function (item) {
	...
	if (typeof item == "object") {
		...
		if (item.hasOwnProperty("streamer_id")) {
			_log(
				"play",
				{ streamer_id: item.streamer_id, link_id: item.link_id || 0, ch_id: item.id },
	...
	try {
		if (this.cur_media_item.error) {
			...
			if (this.mc_proxy_url && match) {
				...
				uri = uri.replace(
					/.+:\/\/[^\/]+/,
					"ffrt " + this.mc_proxy_url + "/" + match[1] + "/" + match[2] + match[3]
				);
				_debug("new uri", uri);
			}
	...
	if (stb.cur_place == "vclub" && this.play_auto_ended) {
		...
		if (this.cur_media_item.media_type == "vclub_ad") {
			...
			this.cur_media_item.video_id &&
				...
				module.vclub.set_ended(this.cur_media_item.video_id);
		}
	}
...
player.prototype.show_rec_icon = function (record) {
	...
	stb.player.rec.set_seconds(
		stb.clock.convert_sec_to_human_time(record["t_start_ts"] - stb.clock.timestamp)
	);
...
player.prototype.tick_s = function (record) {
	stb.player.rec.set_seconds(
		stb.clock.convert_sec_to_human_time(stb.clock.timestamp - record["t_start_ts"])
	);
};
...
player.prototype.disable_pause = function () {
	...
	if (this.active_time_shift) {
		_debug(
			...
			(new Date().getTime() - module.time_shift.cur_media_item.live_date.getTime()) / 1000
		);
	...
	try {
		if (stb.cur_place == "radio") {
			...
			if (
				...
				(this.last_state == 2 &&
					...
					stbPlayerManager.list.length &&
					stbPlayerManager.list[0].state >= 2)
			) {
				if (!skip_button_pos) {
					this.set_pos_button_to_cur_time();
				}
		...
		if (item.cur_series && parseInt(item.cur_series) > 0) {
			this.info.pos_series.innerHTML =
				(item.cur_season ? item.cur_season + " " + get_word("player_season") + ", " : "") +
				item.cur_series +
				" " +
				get_word("player_series");
		} else if (
		...
		if (
			...
			stb.profile.clock_format == "12h" &&
			!this.info.title.haveClass("time_format_12h")
		) {
		...
		this.info.hide_timeout = window.setTimeout(
			function () {
		...
		if (stb.user.fav_itv_on) {
			if (module.tv.genre && module.tv.genre.id != "*") {
				...
				if (stb.profile["hide_tv_genres_in_fullscreen"]) {
					this.f_ch_idx = this._find_nearest_ch_idx(
						dir,
						{ tv_genre_id: stb.profile["hide_tv_genres_in_fullscreen"][0] },
						true
					);
				} else {
			...
			if (module.tv.genre && module.tv.genre.id != "*") {
				...
				if (stb.profile["hide_tv_genres_in_fullscreen"]) {
					this.ch_idx = this._find_nearest_ch_idx(
						dir,
						{ tv_genre_id: stb.profile["hide_tv_genres_in_fullscreen"][0] },
						true
					);
...
player.prototype.set_pos_button = function (to_time) {
	...
	try {
		...
		if (this.cur_media_length != 0) {
			...
			to_pos = Math.round(
				((this.info.pos_bar.offsetWidth - (this.info.pos_button.offsetWidth + 12)) *
					to_time) /
...
player.prototype.move_pos = function (dir) {
	...
	var allow_player_slider_move =
		stbPlayerManager.list[0].state >= 2 && this.cur_media_length >= 0;
	...
	if (
		!allow_player_slider_move &&
		(media_len <= 0 || this.last_state != 4) &&
		!this.active_time_shift
	) {
		return;
	}
...
player.prototype.quick_go_to_ch = function () {
	...
	if (stb.user.fav_itv_on) {
		...
		if (
			stb.profile["hide_tv_genres_in_fullscreen"] &&
			module.tv.genre &&
			module.tv.genre.id != stb.profile["hide_tv_genres_in_fullscreen"][0] &&
			item.tv_genre_id == stb.profile["hide_tv_genres_in_fullscreen"][0]
		) {
			_debug("reset item");
			item = null;
		}
...
player.prototype.change_tv_channel_aspect = function () {
	...
	this.show_aspect_info(
		get_word("aspect_" + stb.aspect_array[this.ch_aspect_idx].alias).toUpperCase()
	);
...
player.prototype.subtitle_pid = {
	...
	get_all: function () {
		...
		if (
			stb.profile["always_enabled_subtitles"] &&
			(stb.user["pri_subtitle_lang"] || stb.user["sec_subtitle_lang"])
		) {
			var picked_subs = this.all_pids.filter(function (item) {
				return (
					...
					((stb.user["pri_subtitle_lang"] &&
						item.lang.indexOf(stb.user["pri_subtitle_lang"]) != -1) ||
						(stb.user["sec_subtitle_lang"] &&
							item.lang.indexOf(stb.user["sec_subtitle_lang"]) != -1) ||
						(stb.user["pri_subtitle_lang"] &&
							item.lang.indexOf(stb.user["pri_subtitle_lang"].slice(0, 2)) != -1) ||
						(stb.user["sec_subtitle_lang"] &&
							item.lang.indexOf(stb.user["sec_subtitle_lang"].slice(0, 2)) != -1))
				);
			});
	...
	init: function () {
		...
		stb.clock.addCustomEventListener("tick", function (date) {
			if (self.on && stb.player.on) {
				self.osd_clock.innerHTML = stb.clock_formats[stb.profile.clock_format].format(
					date.hours,
					date.minutes,
					date.ap_hours,
					date.ap_mark
