...
(function () {
	function epg_constructor() {
			...
			(function () {
				...
				if (!this.active_row["epg_cell"][this.cur_cell_col]) {
					if (!this.player_overlay_mode) {
						...
						ch_idx = stb.player.channels.getIdxById(
							parseInt(this.data_items[this.cur_row].ch_id)
						);
				...
				if (this.active_row["epg_cell"][this.cur_cell_col].data.rec_id) {
					...
					this.active_row["epg_cell"][this.cur_cell_col].data.mark_archive &&
					this.tv_archive
				) {
					...
					if (
						now > this.active_row["epg_cell"][this.cur_cell_col].data.start_timestamp &&
						now < this.active_row["epg_cell"][this.cur_cell_col].data.stop_timestamp
					) {
						if (!this.player_overlay_mode) {
							this.parent.load_params["from_ch_id"] =
								this.data_items[this.cur_row].ch_id;
							this.parent.data_items[this.parent.cur_row].id = 0;
							this.parent.show(true);
							this.hide();
						} else {
							...
							var ch_idx = stb.player.channels.getIdxById(
								parseInt(this.data_items[this.cur_row].ch_id)
							);
							...
							stb.player.last_not_locked_tv_item =
								stb.player.channels[stb.player.ch_idx];
		...
		this.play = function (rec_id) {
			...
			this.active_row["epg_cell"][this.cur_cell_col].data.cmd =
				"auto /media/" + rec_id + ".mpg";
		...
		this.set_date_period = function () {
			...
			var from =
				...
				"-" +
				this.format_date(date_from.getMonth() + 1) +
				"-" +
				this.format_date(date_from.getDate()) +
				" " +
				this.format_date(date_from.getHours()) +
				":" +
				this.format_date(date_from.getMinutes()) +
				":00";
			_debug("from", from);
			...
			var to =
				date_to.getFullYear() +
				"-" +
				this.format_date(date_to.getMonth() + 1) +
				"-" +
				this.format_date(date_to.getDate()) +
				" " +
				this.format_date(date_to.getHours()) +
				":" +
				this.format_date(date_to.getMinutes()) +
				":00";
			_debug("to", to);
		...
		this.fill_dvb_epg = function (data) {
			...
			for (var i = 0; i < data.length; i++) {
				if (data[i].ch_type == "dvb") {
					data[i].epg = module.dvb.get_epg_for_table(
						data[i].dvb_id,
						this.load_params["from_ts"] / 1000,
						this.load_params["to_ts"] / 1000
					);
				}
			}
		...
		this.post_handling_epg_block = function (item, epg, is_active_row) {
			...
			for (var j = 0; j < epg_length; j++) {
				...
				var program_width = Math.floor(
					(container_width * epg[j]["display_duration"]) / 5400
				); // 90m = 5400s
		...
		this.set_active_cell = function () {
			...
			if (this.horiz_dir > 0) {
				...
				if (
					this.active_row["epg_cell"] &&
					empty(this.active_row["epg_cell"][this.cur_cell_col])
				) {
					this.cur_cell_col = this.active_row["epg_cell"].length - 1;
				}
			}
			...
			if (
				!empty(this.active_row["epg_cell"]) &&
				!empty(this.active_row["epg_cell"][this.cur_cell_col])
			) {
				_debug(
					"this.active_row[epg_cell][this.cur_cell_col]",
					this.active_row["epg_cell"][this.cur_cell_col]
				);
				...
				if (now > this.active_row["epg_cell"][this.cur_cell_col].data.start_timestamp) {
					...
					if (
						now >=
							this.active_row["epg_cell"][this.cur_cell_col].data.start_timestamp &&
						module.downloads &&
						this.active_row["epg_cell"][this.cur_cell_col].data.mark_archive &&
						this.tv_archive &&
						!parseInt(this.channel.wowza_dvr, 10)
					) {
						this.color_buttons.get("yellow").enable();
					}
				} else {
					if (
						(module.remote_pvr && this.channel.allow_pvr) ||
						(module.pvr_local && this.channel.allow_local_pvr)
					) {
		...
		this.fill_program_info = function () {
			...
			if (
				...
				this.program_info.innerHTML =
					'<span class="time">' +
					this.active_row["epg_cell"][this.cur_cell_col].data["t_time"] +
					" - " +
					this.active_row["epg_cell"][this.cur_cell_col].data["t_time_to"] +
					"</span> - " +
					this.active_row["epg_cell"][this.cur_cell_col].data["name"] +
				...
				this.on_date.innerHTML =
					this.active_row["epg_cell"][this.cur_cell_col].data["on_date"];
			} else {
				this.program_info.innerHTML = "";
				this.on_date.innerHTML = "";
			}
	...
	epg.init_color_buttons([
		...
		},
		{
			...
			cmd:
				stb.all_modules.indexOf("downloads") >= 0
					? function () {
