...
(function () {
	module.time_shift = {
		...
		get_cur_media_length: function () {
			...
			var live_date =
				cur_piece_date.getYear() +
				"-" +
				cur_piece_date.getMonth() +
				"-" +
				cur_piece_date.getDate();
		...
		get_pos_time: function () {
			...
			if (this.cur_media_item["wowza_dvr"] == 1) {
				var cur_file_date = this._get_wowza_playlist_start_date_by_url(
		...
		_get_wowza_playlist_start_date_by_url: function (url) {
			...
			var true_file_date = file_date_str.replace(
		...
		_get_file_date_by_url: function (url) {
			...
			return new Date(
				new Date(true_file_date).getTime() - stb.profile["timezone_diff"] * 1000
			);
		},
		...
		set_media_item: function (cur_tv_item) {
			...
			_debug(
				"this.cur_media_item.live_date.getTime()",
				this.cur_media_item.live_date.getTime()
			);
			...
			stb.load(
				...
				function (result) {
					...
					for (var i = 0; i < this.program.length; i++) {
						if (
							this.program[i].start_timestamp < now_ts &&
							this.program[i].stop_timestamp > now_ts
						) {
		...
		get_url_by_pos: function (pos) {
			...
			if (stb.player.cur_tv_item["wowza_dvr"] == 1) {
				...
				var url = this.cur_media_item.cmd
					.replace(
						/wowzadvrplayliststart=(\d+)/,
						"wowzadvrplayliststart=" + new_playlist_start
					)
					.trim();
			} else if (this.cur_media_item["flussonic_dvr"] == 1) {
		...
		is_last_archive_day: function () {
			...
			var cur_pos_mark =
				this.cur_piece_date.getYear() +
				"" +
				this.cur_piece_date.getMonth() +
				"" +
				this.cur_piece_date.getDate();
		...
		get_next_part: function () {
			...
			if (stb.player.cur_tv_item["wowza_dvr"] == 1) {
				...
				url = stb.player.cur_media_item.cmd
					.replace(/([^\/]*)\.ts/, next_file_name)
					.replace(/position:(\d*)/, "")
					.trim();
			}
		...
		can_reduce_day: function () {
			...
			_debug(
				"stb.player.cur_media_item.tv_archive_duration",
				stb.player.cur_media_item.tv_archive_duration
			);
		...
		update_position_in_url: function (position) {
			...
			if (!/position:(\d*)/.exec(this.cur_media_item.cmd)) {
				...
				this.cur_media_item.cmd = this.cur_media_item.cmd
			...
			this.on_epg_and_link_available = function () {
				if (module.time_shift.program.length > 0) {
					...
					for (var i = 0; i < module.time_shift.program.length; i++) {
						if (
							module.time_shift.program[i].start_timestamp < now_ts &&
							module.time_shift.program[i].stop_timestamp > now_ts
						) {
		...
		pos_to_previous_program: function () {
			...
			for (var i = 0; i < module.time_shift.program.length; i++) {
				if (
					module.time_shift.program[i].stop_timestamp == this.cur_program.start_timestamp
				) {
			...
			if (
				new Date(this.cur_program.start_timestamp * 1000).getDate() !=
				new Date(cur_program.start_timestamp * 1000).getDate()
			) {
		...
		pos_to_next_program: function () {
			...
			for (var i = 0; i < module.time_shift.program.length; i++) {
				if (
					module.time_shift.program[i].start_timestamp ==
						this.cur_program.stop_timestamp &&
					now_ts > module.time_shift.program[i].start_timestamp
				) {
					cur_program = module.time_shift.program[i];
					break;
				}
