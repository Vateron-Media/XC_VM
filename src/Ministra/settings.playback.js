...
(function () {
	...
	function PlaybackSettingsConstructor() {
		...
		this.save_callback = function (result) {
			...
			stb.SetBufferSize(
				this.save_params.playback_buffer_size * 1000,
				this.save_params.playback_buffer_bytes
			);
		...
		this.save = function () {
			...
			this.save_params.playback_buffer_bytes = Math.round(
				(max_buffer_bytes * playback_buffer_time) / this.max_buffer_time
			);
		...
		this.init = function () {
			...
			this.buffer_changer = this.add_control(
				new VisualValuePickerInput(this, {
			...
			this.audio_out = this.add_control(
			...
			this.playback_limit = this.add_control(
				new OptionInput(this, {
					name: "playback_limit",
	...
	playback_settings.init_color_buttons([
		{
			label: word["playback_settings_cancel"],
			cmd: function () {
