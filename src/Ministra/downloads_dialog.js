...
downloads_dialog_constructor.prototype.show = function (options) {
	...
	var tmp2 = "var tmp = " + stb.RDir("get_storage_info").replace(/\//gm, "").replace(/\n/gm, ""),
		dirs = [];
	eval(tmp2);
	...
	for (i = 0; i < dirs.length; i++) {
		device = dirs[i].replace(/-\d{1,}/gm, "").replace(/USB-/gm, "");
...
downloads_dialog_constructor.prototype.bind = function () {
	...
	(function () {
		if (this.layout == 0) {
			if ($("d_d_button_ok").hasFocus() == true || $("d_d_button_cancel").focused == true) {
				$("d_d_url").focus();
				return;
			}
			if ($("d_d_url").hasFocus() == true) {
	...
	(function () {
		...
		if (this.layout == 0) {
			...
			if (this.layout == 1) {
				$("d_d_fileNameSerial").value = $("dev_" + this.cur_device).getElementsByTagName(
					"input"
				)[0].value;
				$("d_d_fileName").value = $("dev_" + this.cur_device).innerHTML.substr(
					0,
					$("dev_" + this.cur_device).innerHTML.indexOf("|")
				);
				setEnvironmentValue(
					"download_path",
					$("dev_" + this.cur_device).getElementsByTagName("input")[0].value
				);
				$("select_device_shadow").hide();
				$("select_device").hide();
				this.layout = 0;
	...
	for (var i = 0; i < arr.length; i++) {
		...
		div_item.innerHTML =
			...
			(arr[i].info.label != ""
				? "(" + arr[i].info.label + ")"
				: arr[i].num > 0
					? "(#" + arr[i].info.partitionNum + ")"
					: "") +
			" | " +
			(Math.floor(hdd_space.free_mb > 1000)
				? Math.floor(hdd_space.free_gb * 100) / 100 + "Gb"
				: Math.floor(hdd_space.free_mb) + "Mb") +
			" / " +
			(Math.floor(hdd_space.free_mb > 1000)
				? Math.floor(hdd_space.full_gb * 100) / 100 + "Gb"
				: Math.floor(hdd_space.full_mb) + "Mb") +
	...
	if ((set_first && set_first == true) || download_path == "") {
		...
		if (exist_dev == false) {
			...
			if (this.cur_device == 0) {
				...
				$("d_d_fileNameSerial").value = download_path;
				$("d_d_fileName").value =
					arr[tmp_cur_dev].info.vendor +
					" " +
					arr[tmp_cur_dev].info.model +
					(arr[tmp_cur_dev].info.label != ""
						? "(" + arr[tmp_cur_dev].info.label + ")"
						: arr[tmp_cur_dev].num > 0
							? "(#" + arr[tmp_cur_dev].info.partitionNum + ")"
							: "");
			}
		}
	}
...
downloads_dialog_constructor.prototype.moveCursor = function (num) {
	if (num == 1 && this.cur_device >= 0 && this.cur_device != this.count_devices - 1) {
	...
	if (num == -1 && this.cur_device < this.count_devices && this.cur_device != 0) {
	...
	if (
		$("dev_" + (this.visible_devices[0] - 1).toString()) instanceof HTMLDivElement == true &&
	...
	if (
		$("dev_" + (this.visible_devices[2] + 1).toString()) instanceof HTMLDivElement == true &&
...
downloads_dialog_constructor.prototype.init = function () {
	if (!$("downloadsDialogCreate")) {
		...
		dialog.div_outer.divs_inner.main.self.appendChild(
			dialog.div_outer.divs_inner.main.item1.self
		);
		...
		dialog.div_outer.divs_inner.main.item1.self.appendChild(
			dialog.div_outer.divs_inner.main.item1.sub_items.label
		);
		dialog.div_outer.divs_inner.main.item1.sub_items.label.setAttribute("class", "label");
		dialog.div_outer.divs_inner.main.item1.self.appendChild(
			dialog.div_outer.divs_inner.main.item1.sub_items.div_input.self
		);
		dialog.div_outer.divs_inner.main.item1.sub_items.div_input.self.setAttribute(
			"class",
			"input_cover"
		);
		dialog.div_outer.divs_inner.main.item1.sub_items.div_input.input.setAttribute(
			"id",
			"d_d_url"
		);
		dialog.div_outer.divs_inner.main.item1.sub_items.div_input.self.appendChild(
			dialog.div_outer.divs_inner.main.item1.sub_items.div_input.input
		);
		dialog.div_outer.divs_inner.main.self.appendChild(
			dialog.div_outer.divs_inner.main.item2.self
		);
		...
		dialog.div_outer.divs_inner.main.item2.self.appendChild(
			dialog.div_outer.divs_inner.main.item2.sub_items.label
		);
		dialog.div_outer.divs_inner.main.item2.sub_items.label.setAttribute("class", "label");
		dialog.div_outer.divs_inner.main.item2.self.appendChild(
			dialog.div_outer.divs_inner.main.item2.sub_items.div_input.self
		);
		dialog.div_outer.divs_inner.main.item2.sub_items.div_input.self.setAttribute(
			"class",
			"selectbox_cover"
		);
		dialog.div_outer.divs_inner.main.item2.sub_items.div_input.self.setAttribute(
		...
		dialog.div_outer.divs_inner.main.item2.sub_items.div_input.self.appendChild(
			dialog.div_outer.divs_inner.main.item2.sub_items.div_input.input
		);
		dialog.div_outer.divs_inner.main.item2.sub_items.div_input.input.setAttribute(
			"id",
			"d_d_fileName"
		);
		dialog.div_outer.divs_inner.main.item2.sub_items.div_input.input.setAttribute(
		...
		dialog.div_outer.divs_inner.main.item2.sub_items.div_input.self.appendChild(
			dialog.div_outer.divs_inner.main.item2.sub_items.div_input.input_hidden
		);
		dialog.div_outer.divs_inner.main.item2.sub_items.div_input.input_hidden.setAttribute(
			"id",
			"d_d_fileNameSerial"
		);
		dialog.div_outer.divs_inner.main.item2.sub_items.div_input.input_hidden.setAttribute(
		...
		dialog.div_outer.divs_inner.main.self.appendChild(
			dialog.div_outer.divs_inner.main.item3.self
		);
		...
		dialog.div_outer.divs_inner.main.item3.self.appendChild(
			dialog.div_outer.divs_inner.main.item3.sub_items.label
		);
		dialog.div_outer.divs_inner.main.item3.sub_items.label.setAttribute("class", "label");
		dialog.div_outer.divs_inner.main.item3.self.appendChild(
			dialog.div_outer.divs_inner.main.item3.sub_items.div_input.self
		);
		dialog.div_outer.divs_inner.main.item3.sub_items.div_input.self.setAttribute(
			"class",
			"input_cover"
		);
		dialog.div_outer.divs_inner.main.item3.sub_items.div_input.input.setAttribute(
			"id",
			"d_d_fn"
		);
		dialog.div_outer.divs_inner.main.item3.sub_items.div_input.self.appendChild(
			dialog.div_outer.divs_inner.main.item3.sub_items.div_input.input
		);
		dialog.div_outer.divs_inner.main.self.appendChild(
			dialog.div_outer.divs_inner.main.buttons_lay.self
		);
		dialog.div_outer.divs_inner.main.buttons_lay.self.setAttribute("class", "buttons_lay");
		dialog.div_outer.divs_inner.main.buttons_lay.sub_items.button_ok.setAttribute(
			"id",
			"d_d_button_ok"
		);
		dialog.div_outer.divs_inner.main.buttons_lay.sub_items.button_ok.setAttribute(
		...
		dialog.div_outer.divs_inner.main.buttons_lay.self.appendChild(
			dialog.div_outer.divs_inner.main.buttons_lay.sub_items.button_ok
		);
		dialog.div_outer.divs_inner.main.buttons_lay.sub_items.button_cancel.setAttribute(
			"id",
			"d_d_button_cancel"
		);
		dialog.div_outer.divs_inner.main.buttons_lay.sub_items.button_cancel.setAttribute(
		...
		dialog.div_outer.divs_inner.main.buttons_lay.self.appendChild(
			dialog.div_outer.divs_inner.main.buttons_lay.sub_items.button_cancel
		);
		...
		dialog.div_outer.divs_inner.main.item2.sub_items.label.innerHTML =
			get_word("downloads_device") + ":";
		dialog.div_outer.divs_inner.main.item3.sub_items.label.innerHTML =
			get_word("downloads_file_name") + ":";
		dialog.div_outer.divs_inner.main.buttons_lay.sub_items.button_ok.setAttribute(
			"value",
			get_word("downloads_ok")
		);
		dialog.div_outer.divs_inner.main.buttons_lay.sub_items.button_cancel.setAttribute(
			"value",
			get_word("downloads_cancel")
		);
