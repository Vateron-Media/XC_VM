...
(function () {
	...
	function media_browser_constructor() {
		...
		this.init_share_form = function () {
			...
			this.manage_fav_form.addItem(
				new ModalFormInput({
					...
					onchange: function () {
						var server_path = scope.manage_fav_form
							.getItemByName("server_path")
							.getValue();
						var parts = server_path.split("/");
						scope.manage_fav_form
							.getItemByName("local_folder")
				...
				new ModalFormButton({
                     ...
                     */
						...
						var server_path = scope.manage_fav_form
							.getItemByName("server_path")
							.getValue();
						var local_folder = scope.manage_fav_form
							.getItemByName("local_folder")
							.getValue();
						var server_type = scope.manage_fav_form
							.getItemByName("server_type")
							.getValue();
						...
						var fav_item = {
							...
							dir_name:
								local_folder +
								(local_folder.lastIndexOf("/") == local_folder.length - 1
									? ""
									: "/"),
							fav: 1,
							do_not_resolve: 1,
		...
		this.get_smb_passwords = function () {
			...
			if (gSTB.SetScreenSaverImplUrl || gSTB.SetScreenSaverInitAttr) {
				// check if firmware version 0.2.18 and upper
				var smb_data = "[]";
			} else {
		...
		this.init_smb_auth_dialog = function () {
			...
			this.smb_auth_dialog.addItem(
				new ModalFormInput({ label: get_word("smb_username"), name: "login" })
			);
			this.smb_auth_dialog.addItem(
				new ModalFormInput({ label: get_word("smb_password"), name: "password" })
			);
		...
		this.load_mount_data = function (item) {
			...
			if (item && item.hasOwnProperty("full_path")) {
				if (
					item.full_path.indexOf("smb://") == 0 ||
					item.full_path.indexOf("nfs://") == 0
				) {
			...
			for (var i = 0; i < dirs.length; i++) {
				if (!empty(dirs[i])) {
					...
					if (
						name.indexOf("USB-") == 0 ||
						name.indexOf("tmp-smb") == 0 ||
						name.indexOf("SAMBA") == 0
					) {
						continue;
					}
					...
					var dir = {
						name: name,
						dir: 1,
						dir_name: devices[name] ? devices[name].mountPath : dirs[i],
					};
			...
			if (
				new_files &&
				new_files.length &&
				new_files[0].hasOwnProperty("last_modified") &&
				new_files[0].last_modified
			) {
		...
		this.load_smb_groups = function () {
			...
			groups = groups.result.map(function (group) {
				return {
					...
					dir: 1,
					dir_name:
						"SMB_GROUP" /*, "_id" : self.get_id({"name" : group, "dir_name" : "SMB_GROUP"})*/,
				};
			});
			...
			servers = servers.result
				...
				.map(function (server) {
					return {
						...
						dir: 1,
						dir_name:
							"SMB_SERVER" /*, "_id" : self.get_id({"name" : server, "dir_name" : "SMB_SERVER"})*/,
					};
				});
		...
		this.load_smb_shares = function (server) {
			...
			shares = shares.result.shares.map(function (share) {
				return {
					...
					dir: 1,
					dir_name:
						"SMB_SHARE" /*, "_id" : self.get_id({"name" : share, "dir_name" : "SMB_SHARE"})*/,
				};
			});
		...
		this.mount_share = function (type, login, password) {
			...
			if (type == "smb") {
				var mount_str =
					"mount cifs //" +
					Utf8.encode(this.net_server_ip + "/" + this.net_share) +
					" /ram/mnt/smb username=" +
					login +
					",password=" +
					password +
					",iocharset=utf8";
				...
				mount_str =
					...
					this.net_server_ip +
					":/" +
					Utf8.encode(this.net_share + (this.net_path ? "/" + this.net_path : "")) +
					" /ram/mnt/smb ro,nolock";
			}
		...
		this.get_auth_params = function (server, share) {
			...
			return {
				login: this.smb_auth_history[idx].login,
				pass: this.smb_auth_history[idx].pass,
			};
		};
		...
		this.fill_page = function (data) {
			...
			if (stb.player.cur_media_item && this.change_level) {
				var contain_playing_item_idx = this.cur_dir_list.getIdxByVal(
					"cmd",
		...
		this.action = function () {
			if (
				this.data_items[this.cur_row].hasOwnProperty("dir") &&
				!this.data_items[this.cur_row].hasOwnProperty("cmd")
			) {
				...
				var is_image = this.image_extensions.some(function (item) {
					return (
						self.data_items[self.cur_row].name.toLowerCase().lastIndexOf(item) >= 0 &&
						self.data_items[self.cur_row].name.toLowerCase().lastIndexOf(item) ==
							self.data_items[self.cur_row].name.length - item.length
					);
				});

				_debug("is_image", is_image);

				var is_audio = this.audio_extensions.some(function (item) {
					return (
						self.data_items[self.cur_row].name.toLowerCase().lastIndexOf(item) >= 0 &&
						self.data_items[self.cur_row].name.toLowerCase().lastIndexOf(item) ==
							self.data_items[self.cur_row].name.length - item.length
				...
				var is_video = this.video_extensions.some(function (item) {
					return (
						self.data_items[self.cur_row].name.toLowerCase().lastIndexOf(item) >= 0 &&
						self.data_items[self.cur_row].name.toLowerCase().lastIndexOf(item) ==
							self.data_items[self.cur_row].name.length - item.length
				...
				if (is_image) {
					...
					module.image_viewer.show(
						this.image_list,
						this.data_items[this.cur_row].name,
						this.compile_path(),
						this
					);
				} else {
		...
		this.check_for_mount = function (item) {
			...
			var is_smb = this.dir_hist.some(function (dir) {
				return (
					["SMB_GROUP", "SMB_SERVER", "SMB_SHARE"].indexOf(dir.path) >= 0 ||
					(dir.full_path &&
						(dir.full_path.indexOf("smb://") == 0 ||
							dir.full_path.indexOf("nfs://") == 0))
				);
			});
			...
			if (
				item.dir_name == "SMB_SHARE" ||
				(item.hasOwnProperty("full_path") &&
					(item.full_path.indexOf("smb://") == 0 ||
						(item.full_path.indexOf("nfs://") == 0 &&
							(!this.last_mounted ||
								item.full_path.indexOf(this.last_mounted) == -1))) &&
					this.parse_net_path(item).hasOwnProperty("share"))
			) {
				try {
		...
		this.update_breadcrumbs = function () {
			...
			for (var i = 1; i < this.dir_hist.length; i++) {
				if (this.dir_hist[i].path == "SMB_GROUP") {
					...
					["SMB", "SMB_SERVER", "SMB_SHARE", "FAV"].indexOf(this.dir_hist[i].path) >= 0 ||
					this.dir_hist[i].path.indexOf("USB-") === 0 ||
					this.mount_points[this.dir_hist[i].path]
				) {
		...
		this.fill_list = function (data) {
			...
			this.data_items = this.data_items.map(function (item) {
				if (
					...
					item.cmd == stb.player.cur_media_item.cmd &&
					stb.player.on
				) {
		...
		this.get_image_list = function () {
			...
			this.image_list = this.cur_dir_list.filter(function (item) {
				return self.image_extensions.some(function (ext) {
					return (
						item.name.toLowerCase().lastIndexOf(ext) >= 0 &&
						item.name.toLowerCase().lastIndexOf(ext) == item.name.length - ext.length
					);
				});
			});
		...
		this.set_active_row = function (num) {
			...
			if (
				stb.player.cur_media_item &&
				this.data_items[this.cur_row].cmd == stb.player.cur_media_item.cmd &&
				stb.player.on &&
				stb.player.file_type == "audio"
			) {
			...
			if (
				this.dir_hist.length == 2 &&
				this.dir_hist[this.dir_hist.length - 1].path == "FAV"
			) {
			...
			_debug(
				"this.active_row[row].getAttribute(status)",
				this.active_row["row"].getAttribute("status")
			);
		};
		...
		this._update_seek_bar = function () {
			...
			_debug(
				"this.active_row.seek_bar_block.offsetWidth",
				this.active_row.seek_bar_block.offsetWidth
			);
		...
		this.refresh_play_all_switch = function () {
			...
			this.color_buttons.get("red").text_obj.innerHTML =
				get_word("play_all") + ": " + (this.play_all ? get_word("on") : get_word("off"));
		};
		...
		this.add_del_fav = function () {
			...
			if (
				!this.data_items[this.cur_row].hasOwnProperty("dir") ||
				this.data_items[this.cur_row].dir != "1"
			) {
				return;
			}

			var _id =
				this.data_items[this.cur_row]._id || this.get_id(this.data_items[this.cur_row]);
			...
			this.data_items[this.cur_row].full_path =
				this.data_items[this.cur_row].full_path ||
				this.get_full_path(this.data_items[this.cur_row]);
		...
		this.get_full_path = function (item) {
			...
			var is_smb =
				this.dir_hist.some(function (dir) {
	...
	media_browser.init_sort_menu(sort_menu_map, {
		color: "green",
