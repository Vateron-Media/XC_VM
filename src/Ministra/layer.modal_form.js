...
ModalForm.prototype.hide = function () {
	...
	this._items[this._cur_item_idx] &&
		this._items[this._cur_item_idx].blur &&
		this._items[this._cur_item_idx].blur();
	this._dom_obj.hide();
	this.on = false;
	this.reset();
...
ModalForm.prototype._setActiveItem = function () {
	this._items[this._cur_item_idx] &&
		this._items[this._cur_item_idx].focus &&
		this._items[this._cur_item_idx].focus();
};
...
ModalForm.prototype._bind = function () {
	(function (dir) {
		if (
			this._items[this._cur_item_idx] instanceof ModalFormDateTimeSelect &&
			this._items[this._cur_item_idx].inEditMode()
		) {
	...
	(function () {
		if (this._items[this._cur_item_idx] instanceof ModalFormButton) {
			...
			this._items[this._cur_item_idx] instanceof ModalFormSelect ||
			this._items[this._cur_item_idx] instanceof ModalFormDateTimeSelect
		) {
...
ModalFormSelect.prototype.update_input = function () {
	...
	if (
		...
		this._cur_idx >= this._options.length ||
		this._cur_idx == this._options.length - 1
	) {
		this.right_arrow.removeClass("active");
	} else {
...
ModalFormDateTimeSelect.prototype.setValue = function (date) {
	...
	if (this._time_dom_odj) {
		this._time_dom_odj.innerHTML =
			this._formatXX(date.getHours()) + ":" + this._formatXX(date.getMinutes());
	}

	if (this._date_dom_odj) {
		this._date_dom_odj.innerHTML =
			this._formatXX(date.getDate()) +
			"." +
			this._formatXX(date.getMonth() + 1) +
			"." +
			(1900 + date.getYear());
	}
};
