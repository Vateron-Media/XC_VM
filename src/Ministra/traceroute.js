...
function Traceroute(option) {
	...
	this.result = [];
	this.resultCallback =
		option && typeof option.callback == "function" ? option.callback : function () {};
	this.timer = null;
...
Traceroute.prototype.start = function () {
	...
	if (this._time) {
		this._end_time.setSeconds(
			this._start_time.getSeconds() + this._time * this._domains.length + 60
		);
		var self = this;
		this._domains.forEach(function (item) {
			...
			setTimeout(function () {
				...
				result = parent.gSTB.RDir(
					"mtr --report --no-dns --report-cycles " + self._time + " " + item
				);
				result = result.split("\n");
				result.shift();
				result.shift();
...
Traceroute.prototype.getStartTime = function () {
	return arguments[0]
		? this._getNormTime("start")
		: this._start_time instanceof Date
...
Traceroute.prototype.getEndTime = function () {
	return arguments[0]
		? this._getNormTime("end")
		: this._end_time instanceof Date
