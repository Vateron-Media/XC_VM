	...
	if (load_fl) {
		...
		try {
			_GET["dvb_supported_scan_types"] =
				JSON.parse(decodeURIComponent(_GET["dvb_supported_scan_types"])) || [];
		} catch (e) {
		...
		for (var i = 0; i < prof.modules.length; i++) {
			if (
				prof.modules[i].name == "dvb" &&
				(!_GET["dvb_supported_scan_types"] ||
					(Array.isArray(_GET["dvb_supported_scan_types"]) &&
						_GET["dvb_supported_scan_types"].length == 0))
			) {
				continue;
			}
			...
			if (
				gSTB.IsEmulator &&
				["lock", "lang", "playback", "portal", "dev_info", "reload", "reboot"].indexOf(
					prof.modules[i].name
				) == -1
			) {
				continue;
			...
			if (
				(!gSTB.ConfigNetRc || !gSTB.SetNetRcStatus) &&
				prof.modules[i].name == "remote_control"
			) {
				continue;
			}
	...
	switch (urlP) {
		case 0:
			switch (punkti[vid - 1][4]) {
				case 1:
					conf(
						t("Do you want to go to the inner portal?"),
						"",
						'stb.Stop();stb.LoadURL("file:///home/web/services.html");stbWebWindow.close();'
					);
					break;
				case 2:
					conf(
						t("Do you want to restart portal?"),
						"",
						'stb.Stop();stb.LoadURL("file:///home/web/index.html");stbWebWindow.close();'
					);
					break;
				case 3:
					conf(
						t("Device is going to reboot. Are you sure?"),
						"",
				...
				default:
					...
					document.getElementById("ico").innerHTML =
						"<img src='style/" + put + "/" + punkti[vid - 1][3] + ".png' />";
					break;
			}
			break;
		case "glavnaya.html":
			...
			document.getElementById("ico").innerHTML =
				"<img src='style/" + put + "/ico_l_set.png' />";
			break;
		default:
			cont.style.visibility = "hidden";
			cont.src = urlP;
...
function erMes(erm, el) {
	...
	d.innerHTML =
		'<div id="modal_ico" class="ico_alert"></div>' +
		erm +
		'<div id="modal_btn"><input id="b1" type="button" value="' +
		t("Ok") +
		'" onClick="erMbeck()" /></div>';
	document.getElementById("b1").focus();
}
...
function conf(erm, el, ef, back) {
	...
	document.getElementById("b1").focus();
	d.innerHTML =
		'<div id="modal_ico" class="ico_issue"></div>' +
		erm +
		'<div id="modal_btn"><input id="b2" type="button" value="' +
		t("Cancel") +
		'" onClick="erMbeck()" /><input id="b1" type="button" value="' +
		t("Ok") +
		'" onClick="confOk()" /></div>';
...
function confb(erm, el, ef, back) {
	...
	d.innerHTML =
		'<div id="modal_ico" class="ico_issue"></div>' +
		erm +
		'<div id="modal_btn"><input id="b2b" type="button" value="' +
		t("Cancel") +
		'" onClick="erMbeck()" /><input id="b1b" type="button" value="' +
		t("Ok") +
		'" onClick="confOk()" /></div>';
...
function confOk() {
	eval(func);
	if (
		document.getElementById("confirmb").style.display == "block" ||
