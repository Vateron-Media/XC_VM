(function () {
	...
	function ServiceManagement() {
		...
		this.init = function () {
			...
			this.confirm_unsubscribe_dialog.addItem(
				new ModalFormButton({
					value: get_word("cancel_btn"),
					onclick: function () {
			...
			this.complete_confirm.addItem(
				new ModalFormButton({
					...
					onclick: function () {
						...
						_debug(
							"scope.complete_confirm.need_reboot",
							scope.complete_confirm.need_reboot
						);
			...
			this.password_input = new ModalForm({
				title: get_word("parent_password_title"),
				parent: main_menu,
			});
			this.password_input.enableOnExitClose();

			this.password_input.addItem(
				new ModalFormInput({
					label: get_word("password_label"),
					name: "password_input",
					type: "password",
			...
			this.password_input.addItem(
				new ModalFormButton({
					value: get_word("ok_btn"),
					onclick: function () {
						var password_input = scope.password_input
							.getItemByName("password_input")
							.getValue();
		...
		this.set_active_row = function (num) {
			...
			if (!stb.profile["allow_subscription_from_stb"]) {
				...
				this.data_items[this.cur_row].optional &&
				!this.data_items[this.cur_row].subscribed
			) {
				...
				function (result) {
					...
					if (!result) {
						...
						if (!/[^0-9\.,]/.test(this.price_confirm.price)) {
							this.price_confirm.price =
								this.price_confirm.price + get_word("package_price_measurement");
						}

						this.price_confirm.tariff_package = tariff_package;
						_debug(
							"this.price_confirm.tariff_package",
							this.price_confirm.tariff_package
						);
						this.price_confirm.show(
							get_word("confirm_service_price_text").format(this.price_confirm.price)
						);
					}
				},
				...
				function (result) {
					...
					if (!result || (result.hasOwnProperty("result") && result.result === 0)) {
						...
						if (tariff_package.type == "module") {
							...
							this.complete_confirm.show(
								get_word("service_subscribe_success_reboot")
							);
				...
				function (result) {
					...
					if (!result || (result.hasOwnProperty("result") && result.result === 0)) {
						...
						if (tariff_package.type == "module") {
							this.complete_confirm.need_reboot = true;
							this.complete_confirm.show(
								get_word("service_unsubscribe_success_reboot")
							);
						} else if (tariff_package.type == "tv") {
							this.complete_confirm.need_reboot = false;
							stb.load_channels();
	...
	function PackageInfoConstructor(parent) {
		...
		this.init = function () {
			...
			this.main_container.content = new Scrollable(
				create_block_element("main_content", this.main_container),
				this.main_container
			);
		...
		this.hide = function () {
			...
			this.main_container.content.scrollbar &&
				this.main_container.content.scrollbar.reset &&
				this.main_container.content.scrollbar.reset();
		};
		...
		this.fill_info = function (info) {
			...
			if (info["type"]) {
				info_str +=
					'<div class="info-block"><span class="label">' +
					get_word("package_type") +
					":</span> " +
					info["type"] +
					"</div>";
			}
	...
	service_management.init_color_buttons([
		{
			label: get_word("UNSUBSCRIBE"),
			cmd: function () {
				service_management.unsubscribe();
			},
		},
		{
			label: get_word("SUBSCRIBE"),
			cmd: function () {
	...
	service_management.update_header_path([
		{ alias: "tab", item: get_word("SERVICES MANAGEMENT") },
	]);
