...
(function () {
	...
	function downloads_constructor() {
		...
		this.adjust_priority = function (id, rise) {
			...
			var active_queue = eval(stbDownloadManager.GetQueueInfo());
			var in_active_queue_idx = active_queue.getIdxByVal(
				"filePath",
				this._queue[idx].filePath
			);
			...
			if (rise) {
				if (in_active_queue_idx !== null) {
					stbDownloadManager.AdjustJobPriority(
						active_queue[in_active_queue_idx].id,
						true
					);
				}
		...
		this.del = function (id, del_file) {
			...
			if (idx !== null) {
				if (del_file) {
					...
					var remove_result = stb.RDir(
						'RemoveFile "' + this.get_full_file_path(this._queue[idx]) + '"'
					);
		...
		this.every_interval = function (do_load_data) {
			...
			if (stbDownloadManager) {
				...
				if (clear_queue.length > 0 && !active_download_exits) {
					...
					this.get_url(clear_queue[0], function (cmd) {
						...
						self._queue[idx].filePath = self.update_file_path(
							self._queue[idx].filePath,
							cmd
						);

						_debug("stbDownloadManager.AddJob", cmd, self._queue[idx].filePath);

						var result = stbDownloadManager.AddJob(
							cmd,
							self.get_full_file_path(self._queue[idx])
						);
			...
			for (var i = 0; i < obj.length; i++) {
				...
				this.forFill.push({
					...
					d_progress:
						obj[i].progressPct != "-1"
							? '<span class="pb"><span style="width:' +
								(obj[i].progressPct * 110) / 100 +
								'px;"></span></span><span class="txt">' +
								obj[i].progressPct.substr(0, 4) +
								"%</span>"
	...
	downloads.init_color_buttons([
		{
			label: get_word("downloads_create"),
			cmd: function () {
				this.drawCreateDialog();
			},
		},
		{
			label: get_word("downloads_move_up"),
			cmd: function () {
		...
		{
			label: get_word("downloads_move_down"),
			cmd: function () {
				//if (downloads.data_items[downloads.cur_row].fake){
