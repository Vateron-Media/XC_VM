...
(function () {
	function EpgSimpleConstructor() {
		...
		this.init = function () {
			...
			this.full_program_info_container = create_block_element(
				"full_program_info",
				this.dom_obj
			);
			this.full_program_info = new Scrollable(
				create_block_element("full_program_info_content", this.full_program_info_container),
				this.full_program_info_container
			);
			...
			(function () {
				if (this.more_on) {
					...
					if (this.data_items[this.cur_row].rec_id) {
						...
						this.data_items[this.cur_row].start_timestamp < now &&
						this.data_items[this.cur_row].stop_timestamp > now
					) {
		...
		this.load_data = function () {
			...
			if (this.channel.type == "dvb") {
				...
				var result = module.dvb.get_epg_for_page(
					this.channel["dvb_id"],
					date,
					this.cur_page,
					this.total_rows
		...
		this.set_active_row = function (num) {
			...
			_debug("now", now);
			_debug(
				"this.data_items[this.cur_row].start_timestamp",
				this.data_items[this.cur_row].start_timestamp
			);
			...
			if (this.channel.open == 0) {
				...
				_debug(
					"this.data_items[this.cur_row].stop_timestamp",
					this.data_items[this.cur_row].stop_timestamp
				);
				_debug(
					"this.data_items[this.cur_row].mark_archive",
					!!this.data_items[this.cur_row].mark_archive
				);
				_debug(
					"!stb.player.cur_media_item.wowza_dvr",
					!stb.player.cur_media_item.wowza_dvr
				);
				_debug("module.downloads", !!module.downloads);
				_debug("this.tv_archive", !!this.tv_archive);
				_debug(
					"now >= this.data_items[this.cur_row].stop_timestamp",
					now >= this.data_items[this.cur_row].stop_timestamp
				);
				...
				if (
					...
					!!this.data_items[this.cur_row].mark_archive &&
					!!this.tv_archive &&
					!parseInt(stb.player.cur_media_item.wowza_dvr, 10)
				) {
		...
		this.fill_program_info = function () {
			...
			this.fill_info_timer = window.setTimeout(
				function () {
					if (!empty(self.data_items) && !empty(self.data_items[self.cur_row])) {
						self.program_info.innerHTML =
							...
							"</span> - " +
							(self.data_items[self.cur_row]["o_name"]
								? self.data_items[self.cur_row]["o_name"]
								: self.data_items[self.cur_row]["name"]) +
							(self.data_items[self.cur_row]["descr"]
								? " (" + self.data_items[self.cur_row]["descr"] + ")"
								: "");
					} else {
						self.program_info.innerHTML = "";
		...
		this.week_day_action = function () {
			...
			this.load_params["date"] =
				this.week[this.cur_week_row + (this.cur_week_page - 1) * 7].f_mysql;
		...
		this.more_show = function () {
			...
			if (this.data_items && this.data_items[this.cur_row]) {
				this.full_program_info.dom_obj.innerHTML =
					...
					"</span> - " +
					(this.data_items[this.cur_row]["o_name"]
						? this.data_items[this.cur_row]["o_name"]
						: this.data_items[this.cur_row]["name"]) +
					"<br>" +
					(this.data_items[this.cur_row]["category"]
						? ' <br><span class="time">' +
							get_word("epg_category") +
							"</span>: " +
							this.data_items[this.cur_row]["category"] +
							""
						: "") +
					(this.data_items[this.cur_row]["director"]
						? ' <br><span class="time">' +
							get_word("epg_director") +
							"</span>: " +
							this.data_items[this.cur_row]["director"] +
							""
						: "") +
					(this.data_items[this.cur_row]["actor"]
						? ' <br><span class="time">' +
							get_word("epg_actors") +
							"</span>: " +
							this.data_items[this.cur_row]["actor"] +
							""
						: "") +
					(this.data_items[this.cur_row]["descr"]
						? ' <br><span class="time">' +
							get_word("epg_desc") +
							"</span>: " +
							this.data_items[this.cur_row]["descr"] +
			...
			},
			{
				...
				cmd:
					stb.all_modules.indexOf("downloads") >= 0
						? function () {
								epg_simple.tv_archive.play.call(
									epg_simple.tv_archive,
									true,
									function (url, to_file) {
