...
function common_xpcom() {
	...
	this.get_server_params = function () {
		...
		this.ajax_loader =
			this.portal_protocol + "://" + this.portal_ip + "/" + this.portal_path + "/portal.php";
	...
	this.is_restricted_module = function (module) {
		...
		if (
			!this.additional_services_on &&
			this.switchable_modules.indexOf(module.layer_name) >= 0
		) {
			return true;
		}
	...
	this.load = function (params, var_args) {
		...
		if (
			...
			this.get_debug_param("auth_via_query") == "1" &&
		...
		try {
			...
			if (method == "POST") {
				req.open(
					"POST",
					this.ajax_loader + "?JsHttpRequest=" + new Date().getTime() + "-xml",
					true
				);
		...
		this.load(
			{
				...
				hw_version_2: stb.GetHashVersion1
					? stb.GetHashVersion1(JSON.stringify(metrics), this.random)
					: "",
				timestamp: Math.round(new Date().getTime() / 1000),
				api_signature:
					typeof gSTB == "undefined"
						? 0
						: (function () {
								var p = 0;
								for (var d in gSTB) {
	...
	this.check_image_version = function () {
		...
		_debug(
			"stb.user[autoupdate] is array",
			stb.user["autoupdate"] && stb.user["autoupdate"] instanceof Array
		);
	...
	this.check_update = function (params) {
		...
		if (
			...
			((params.require_image_version != "" &&
				params.require_image_version != this.image_version) ||
				(params.require_image_date != "" &&
					params.require_image_date != this.image_date)) &&
			(params.image_version_contains == "" ||
				params.image_version_contains == this.image_version) &&
			(params.image_description_contains == "" ||
				this.image_desc.indexOf(params.image_description_contains) != -1) &&
			(params.hardware_version_contains == "" ||
				this.hw_version.indexOf(params.hardware_version_contains) != -1)
		) {
			...
			if (
				...
				try {
					...
					this.user["update_url"] = this.user["update_url"].replace(
						/\/imageupdate$/,
						"/" + params.prefix + "imageupdate"
					);
	...
	this.user_init = function (user_data) {
		...
		if (
			this.user["deny_720p_gmode_on_mag200"] &&
			this.type == "MAG200" &&
			screen.height >= 720
		) {
		...
		if (this.user["status"] == 2) {
			...
			if (
				this.user["template"] &&
				this.user["template"].indexOf("smart_launcher") != -1 &&
				single_module.length == 0
			) {
				_debug("redirect to the new launcher");
				window.stop();
				document.body.hide();
				_debug(
					this.user["launcher_url"] +
						"?config=" +
						encodeURIComponent(
							this.user["launcher_profile_url"] +
								"?uid=" +
								this.user["id"] +
								"&language=" +
								this.stb_lang_orig +
			...
			try {
				...
				stb.player.ch_aspect_idx = this.aspect_array.getIdxByVal(
					"mode",
					this.user["aspect"]
				);
				...
				this.user["tv_playback_retry_limit"] = parseInt(
					this.user["tv_playback_retry_limit"],
					10
				);
				...
				this.profile["plasma_saving_timeout"] = parseInt(
					this.profile["plasma_saving_timeout"],
					10
				);
				...
				var aspect_idx = this.aspect_array.getIdxByVal(
					"alias",
				...
				if (!this.user["pri_audio_lang"]) {
					this.user["pri_audio_lang"] = this.lang_map.hasOwnProperty(this.stb_lang)
						? this.lang_map[this.stb_lang]
						: "";
					this.user["sec_audio_lang"] = this.lang_map.hasOwnProperty(this.user.stb_lang)
						? this.lang_map[this.user.stb_lang]
						: "";
					...
					if (this.user["pri_audio_lang"] == this.user["sec_audio_lang"]) {
						...
						this.user["sec_audio_lang"] = this.lang_map.hasOwnProperty(default_lang)
							? this.lang_map[default_lang]
							: "";
					}
				}
				...
				try {
					...
					if (this.user["web_proxy_host"]) {
						stb.SetWebProxy &&
							stb.SetWebProxy(
								this.user["web_proxy_host"],
								this.user["web_proxy_port"],
								this.user["web_proxy_user"],
								this.user["web_proxy_pass"],
					...
					if (
						gSTB.SetLedIndicatorLevels &&
						this.user.hasOwnProperty("default_led_level") &&
						this.user.hasOwnProperty("standby_led_level")
					) {
						_debug(
							"this.user[default_led_level]",
							parseInt(this.user["default_led_level"], 10)
						);
						_debug(
							"this.user[standby_led_level]",
							parseInt(this.user["standby_led_level"], 10)
						);
					...
					if (
						typeof stb.SetCheckSSLCertificate == "function" &&
						typeof this.profile["check_ssl_certificate"] != "undefined" &&
						this.profile["check_ssl_certificate"] == 0
					) {
			...
			if (
				this.ntp_server &&
				this.user.hasOwnProperty("ntp_wait_timeout") &&
				this.ntp_wait_time <= this.user["ntp_wait_timeout"]
			) {
				_debug("wait for ntp");
	...
	this.mount_home_dir = function (storages) {
		...
		for (var i in storages) {
			if (storages.hasOwnProperty(i)) {
				...
				var mount_cmd =
					'"' +
					storages[i]["storage_ip"] +
					":" +
					storages[i]["nfs_home_path"] +
					stb.mac +
					'" /media/' +
					storages[i]["storage_name"];
	...
	this.Mount = function (link_cmd) {
		...
		try {
			...
			var mount_cmd =
				'"' +
				this.storages[this.mounted_storage]["storage_ip"] +
				":" +
				this.storages[this.mounted_storage]["nfs_home_path"] +
				this.mac +
				'" /media/' +
	...
	this.resize_window = function () {
		...
		try {
			...
			if (res["r" + gres]) {
				...
				_debug(
					"window.moveTo",
					(res["r" + gres].w - res["r" + gres].window_w) / 2,
					(res["r" + gres].h - res["r" + gres].window_h) / 2
				);
				window.moveTo(
					(res["r" + gres].w - res["r" + gres].window_w) / 2,
					(res["r" + gres].h - res["r" + gres].window_h) / 2
		...
		this.load(
			...
			function (result) {
				...
				if (this.player.is_tv) {
					var ch_idx = this.player.channels.getIdxByVal(
						"id",
						this.player.cur_media_item.id
					);
				...
				stb.player.on_play = function (ch_id) {
					...
					if (stb.player.is_tv) {
						...
						if (rec_idx !== null) {
							...
							if (
								stb.recordings[rec_idx].local == 0 ||
								(stb.recordings[rec_idx].t_start_ts < now_ts &&
									stb.recordings[rec_idx].t_stop_ts > now_ts)
							) {
		...
		get_curr_and_next: function (ch_id, from_ts, length) {
			...
			if (length > 2 && module.epg_reminder && Array.isArray(module.epg_reminder.memos)) {
				for (i = 0; i < result.length; i++) {
					result[i]["mark_memo"] =
						module.epg_reminder.memos.getIdxByVal("tv_program_id", result[i]["id"]) !=
						null
							? 1
							: 0;
				}
	...
	this.get_current_place = function () {
		...
		if (this.player.media_type == "stream") {
			// TV
			if (this.player.on) {
				...
				} else if (this.cur_place == "radio") {
					// Radio
					cur_place_num = 5;
				} else if (this.cur_place == "vclub") {
					cur_place_num = 2;
				} else if (this.cur_place == "karaoke") {
					// Karaoke
					cur_place_num = 3;
				} else if (this.cur_place == "audioclub") {
					// Audio Club
					cur_place_num = 4;
			...
			if (this.player.on) {
				if (this.cur_place == "vclub") {
					...
					// Audio Club
					cur_place_num = 4;
				} else if (this.cur_place == "video_clips") {
					// Video Clips
					cur_place_num = 8;
	...
	this.advert = {
			...
			stb.load(
				...
				function (result) {
					...
					var adverts = result.filter(function (item) {
						return (
							item["campaign"].hasOwnProperty("places") &&
							item["campaign"]["places"] &&
							item["campaign"]["places"][101]
						);
					});
		...
		start_ticking: function (media_len) {
			...
			this.ticking_timeout = window.setInterval(function () {
				...
				if (
					Math.abs(quartile - pos_time) <= 1 &&
					stb.player.cur_media_item.ad_tracking.hasOwnProperty("firstQuartile")
				) {
					stb.advert.track(stb.player.cur_media_item.ad_tracking["firstQuartile"]);
				} else if (
					Math.abs(2 * quartile - pos_time) <= 1 &&
					stb.player.cur_media_item.ad_tracking.hasOwnProperty("midpoint")
				) {
					stb.advert.track(stb.player.cur_media_item.ad_tracking["midpoint"]);
				} else if (
					Math.abs(3 * quartile - pos_time) <= 1 &&
					stb.player.cur_media_item.ad_tracking.hasOwnProperty("thirdQuartile")
				) {
	...
	this.clock = {
		...
		show: function () {
			if (
				...
				main_menu.date.innerHTML = get_word("date_format").format(
					get_word("week_arr")[this.day],
					this.date,
					get_word("month_arr")[this.month],
					this.year
				);
			}
...
var connection_problem = {
	...
	show: function () {
		...
		_debug(
			"stb.user.enable_connection_problem_indication",
			stb.user.enable_connection_problem_indication
		);
