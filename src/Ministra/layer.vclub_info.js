...
vclub_info.prototype.hide = function () {
	...
	this.parent.password_input.on &&
		this.parent.password_input.hide &&
		this.parent.password_input.hide();
	this.parent.rent_password_input.on &&
		this.parent.rent_password_input.hide &&
		this.parent.rent_password_input.hide();
	this.parent.series_switch.on &&
...
vclub_info.prototype.fill = function (item) {
	...
	this.film_title.innerHTML =
		item.name == item.o_name ? item.name : item.name + " / " + item.o_name;

	var full_info =
		"<span>" + word["vclub_year"] + ":</span> " + item.year.substring(0, 4) + " &nbsp; ";
	...
	full_info +=
		...
		item.time +
		" " +
		(empty(item.series) ? word["vclub_minutes"] + "." : "") +
		"<br>";
	full_info +=
