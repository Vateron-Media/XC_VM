...
watchdog.prototype.run = function (timeout, timeslot) {
	...
	_debug(
		"Math.floor((now - day_start)/this.request_timeout)",
		Math.floor((now - day_start) / this.request_timeout)
	);
	...
	var delay =
		...
		(now -
			day_start -
			Math.floor((now - day_start) / this.request_timeout) * this.request_timeout);
	...
	if (typeof data.id != "undefined") {
		...
		switch (data.event) {
			...
			case "send_msg": {
				...
				if (
					data.event == "send_msg_with_video" ||
					data.post_function == "send_msg_with_video"
				) {
					...
					msg_callback = function () {
						...
						if (stbWindowMgr.openWebFace) {
							...
							stbWindowMgr.openWebFace(
								path +
									"public/app/ibman/index.html?mode=2&url=" +
				...
				stb.msg.push({
					msg:
						(data.send_time
							? '<span style="color: #555">[' + data.send_time + "]</span> "
							: "") +
						data.msg.replace("%mac%", stb.mac).replace("%sn%", stb.serial_number),
