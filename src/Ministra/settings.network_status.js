...
(function () {
	...
	function NetworkStatusConstructor() {
		...
		this.refresh = function () {
			...
			this.list.updateValueByTitle(
				get_word("LAN"),
				stb.GetLanLinkStatus()
					? '<span class="if_up">' + get_word("lan_up") + "</span>"
					: '<span class="if_down">' + get_word("lan_down") + "</span>"
			);
			...
			speedtest.onCheck(function (result) {
				if (result.state == 2) {
					self.list.updateValueByTitle(
						get_word("test_speed"),
						get_word("speedtest_testing")
					);
				} else if (result.state == 4 || result.state == 5) {
					self.list.updateValueByTitle(
						get_word("test_speed"),
						get_word("speedtest_error")
					);
				} else if (result.state == 1) {
					self.list.updateValueByTitle(
						get_word("test_speed"),
						get_word("speedtest_waiting")
					);
				}
			});
	...
	network_status.init_color_buttons([
		{
			label: get_word("network_status_refresh"),
			cmd: function () {
