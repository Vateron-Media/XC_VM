var main_menu = {
	...
	show: function () {
		...
		var layer_name =
			this.map.length >= 1 && this.map[1].module && this.map[1].module.layer_name
				? this.map[1].module.layer_name
				: null;
	...
	init: function () {
		...
		this.exit_comfirm = new ModalForm({
			title: get_word("confirm_form_title"),
			text: get_word("exit_title"),
		});
		...
		this.settings_password_promt.addItem(
			new ModalFormInput({
		...
		this.settings_password_promt.addItem(
			new ModalFormButton({
				...
				onclick: function () {
					...
					var settings_password = scope.settings_password_promt
						.getItemByName("settings_password")
						.getValue();
					...
					if (settings_password == profile_settings_password) {
						...
						scope.settings_password_promt.callback &&
							scope.settings_password_promt.callback();
					} else {
	...
	action: function () {
		...
		if (
			this.map[1].sub &&
			this.map[1].sub[this.active_sub] &&
			typeof this.map[1].sub[this.active_sub].cmd == "object"
		) {
			var context = this.map[1].sub[this.active_sub].cmd.context || window;

			stb.advert.start(function () {
				self.map[1].sub[self.active_sub].cmd.func.apply(
					context,
					self.map[1].sub[self.active_sub].cmd.args
				);
			});
		} else if (
			this.map[1].sub &&
			this.map[1].sub[this.active_sub] &&
			typeof this.map[1].sub[this.active_sub].cmd == "function"
		) {
			stb.advert.start(function () {
			...
			this.map[1].sub &&
			this.map[1].sub[this.active_sub] &&
			typeof this.map[1].sub[this.active_sub].cmd == "string"
		) {
			stb.advert.start(function () {
			...
			if (
				self.map[1] &&
				self.map[1].module &&
				self.map[1].module.layer_name &&
				self.map[1].module.layer_name == "settings"
			) {
				self.map[1].cmd();
			} else {
				stb.advert.start(function () {
	...
	add: function (title, sub, img, cmd, module) {
		...
		this.map.push({
			...
			img:
				img.indexOf("http") == 0
					? img
					: "template/" + loader.template + "/i" + resolution_prefix + "/" + img,
			cmd: cmd,
			sub: sub,
			module: module,
			idx: idx,
