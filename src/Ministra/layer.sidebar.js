...
sidebar.prototype.reset = function () {
	...
	for (var i = 0; i < this.lists.length; i++) {
		...
		this.parent.update_header_path([
			{ alias: this.lists[i].alias, item: this.lists[i].selected_title },
		]);
	}
...
sidebar.prototype.vshift = function (dir) {
	...
	if (dir > 0) {
		...
		this.lists[this.cur_list_idx].map.unshift(
			this.lists[this.cur_list_idx].map[menu_length - 1]
		);
...
sidebar.prototype.set_active_list = function () {
	for (var i = 0; i < this.lists.length; i++) {
		if (i != this.cur_list_idx) {
			...
			this.arrows.moveX(
				this.lists[i].dom_obj.clientWidth / 2 - 6 + this.lists[i].dom_obj.offsetLeft
			);
		}
	}
};
