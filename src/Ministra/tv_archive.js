	...
	module.tv_archive = {
		...
		get_part_by_pos: function (new_pos) {
			...
			var start_date = new Date(
				parseInt(stb.player.cur_media_item.start_timestamp, 10) * 1000
			);
			...
			_debug(
				"this.get_filename_by_date(this._get_file_date_by_url(stb.player.cur_media_item.cmd))",
				this.get_filename_by_date(this._get_file_date_by_url(stb.player.cur_media_item.cmd))
			);

			if (
				this.get_filename_by_date(
					this._get_file_date_by_url(stb.player.cur_media_item.cmd)
				) == new_file_name
			) {
				return false;
			}
		...
		get_next_part: function () {
			...
			var url = stb.player.cur_media_item.cmd
		...
		get_pos_time: function () {
			...
			if (file_num == 0) {
				...
				pos_time =
					current_pos_time + (3600 - this.get_start_position()) + (file_num - 1) * 3600;
			}
