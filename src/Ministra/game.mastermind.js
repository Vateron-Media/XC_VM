...
(function () {
	function mastermind_constructor() {
		...
		this.check_guess = function () {
			...
			if (
				this.fields_map[0].innerHTML == "" ||
				this.fields_map[1].innerHTML == "" ||
				this.fields_map[2].innerHTML == "" ||
				this.fields_map[3].innerHTML == ""
			) {
				return;
			}
		...
		this.add_log = function (step) {
			...
			this.log.push({
				...
				guess: step.guess,
				score:
					step.bulls + word["mastermind_bull"] + " " + step.cows + word["mastermind_cow"],
			});
	...
	function rules_constructor() {
		...
		this.init = function () {
			...
			text +=
				'<br><br><table width="100%" cellpadding="3" cellspacing="3"><tr><td width="100" style="color:#ffffff;">&larr; &rarr;</td><td style="color:#3f81cc;">' +
				word["mastermind_move_cursor"] +
				"</td></tr>";
			text +=
				'<tr><td style="color:#ffffff;">0-9</td><td style="color:#3f81cc;">' +
				word["mastermind_cell_numbers"] +
				"</td></tr>";
			text +=
				'<tr><td style="color:#ffffff;">OK</td><td style="color:#3f81cc;">' +
				word["mastermind_step_confirmation"] +
				"</td></tr>";
			text +=
				'<tr><td style="color:#ffffff;">' +
				word["mastermind_page"] +
				' +/-</td><td style="color:#3f81cc;">' +
				word["mastermind_history_moves"] +
				"</td></tr></table>";
	...
	mastermind.init_color_buttons([
		{
			label: word["mastermind_rules"],
			cmd: function () {
				mastermind.rules.show.apply(mastermind.rules);
			},
		},
		{
			label: word["mastermind_rating"],
			cmd: function () {
