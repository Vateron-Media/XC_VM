...
(function () {
	...
	var allow_local_recording = stb.user["allowed_stb_types_for_local_recording"].some(
		function (element) {
	...
	function PvrLocal() {
				...
				if (now - stb.recordings[idx].t_start_ts < 120) {
					if (this.duration_input.on) {
						...
						this.duration_input.callback = function (duration) {
							...
							var t_stop_ts =
								parseInt(stb.recordings[idx].t_start_ts, 10) + duration * 60;

							var result = pvrManager.ChangeEndTime(
								stb.recordings[idx].internal_id,
								t_stop_ts
							);
							...
							var del_timeout =
								(stb.recordings[idx].t_stop_ts - stb.recordings[idx].t_start_ts) *
								1000;
		...
		this._get_link_for_channel = function (channel, callback) {
			...
			if (
				parseInt(channel.use_http_tmp_link) == 1 ||
				parseInt(channel.use_load_balancing) == 1 ||
		...
		this.start = function (channel, path, start, end, program, callback) {
			...
			var self = this;
			record["del_timeout"] = window.setTimeout(
				function () {
					_debug("delete rec");
			...
			stb.load(
				...
				function (result) {
					...
					if (program) {
						record["start_timeout"] = window.setTimeout(
							function () {
								_debug("start rec");

								self._get_link_for_channel(channel, function (url) {
									...
									stb.recordings[idx].internal_id = pvrManager.CreateTask(
										url,
										path,
										start,
										end
		...
		this.get_record_filename_for_program = function (program) {
			...
			return (
				...
				"_" +
				(channel_name + "_" + cur_prog).toTranslit() +
				".mpg"
			);
		};
		...
		this.get_record_filename = function (channel) {
			...
			return (
				...
				"_" +
				(channel.name + "_" + cur_prog).toTranslit() +
				".mpg"
			);
		};
	...
	stb.recordings.map(function (record) {
		...
		if (
			record.local == 1 &&
			active_tasks.getIdxByVal("fileName", record.file) === null &&
			now_ts > record.t_start_ts
		) {
			// remove old
			_debug("remove old", record);

			module.pvr_local.send_stop_rec(record.id);
		} else if (record.local == 1 && now_ts < record.t_start_ts) {
			// deferred
		...
		} else if (record.local == 1 && now_ts > record.t_start_ts && now_ts < record.t_stop_ts) {
			// restore current
