(function () {
	if (
		...
		JSON.parse(dvbManager.GetSupportedScanTypes()).length == 0
	) {
		return;
	}
	...
	module.dvb = {
		...
		manual_scan: function (dvb_params) {
			...
			if (typeof dvb_params == "object") {
				dvbManager.SetScanParams(JSON.stringify(dvb_params));
				dvbManager.StartChannelScanManual(
					dvb_params.frequency,
					dvb_params.frequency,
					dvb_params.type,
					dvb_params.bandwidth,
					1000
				);
			}
		},
		...
		update_epg: function (dvb_ch_id) {
			...
			if (epg.length > 0) {
				epg = epg.map(function (program) {
					var end_timestamp =
						parseInt(program.start, 10) + parseInt(program.duration, 10);
		...
		get_epg_for_page: function (dvb_ch_id, date, page_number, items_per_page) {
			...
			if (stb.epg_loader.epg[ch_id]) {
				...
				for (var i = 0; i < length; i++) {
					...
					if (
						default_selected &&
						stb.epg_loader.epg[ch_id][i].start_timestamp < now_ts &&
						stb.epg_loader.epg[ch_id][i].stop_timestamp > now_ts
					) {
		...
		get_epg_for_table: function (dvb_ch_id, from_ts, to_ts) {
			...
			for (var i = 0; i < length; i++) {
				if (
					stb.epg_loader.epg[ch_id][i].start_timestamp < to_ts &&
					stb.epg_loader.epg[ch_id][i].stop_timestamp > from_ts
				) {
					...
					if (program["start_timestamp"] < from_ts) {
						program["larr"] = 1;
						program["display_duration"] =
							program["duration"] - (from_ts - program["start_timestamp"]);
					}

					if (program["stop_timestamp"] > to_ts) {
						program["rarr"] = 1;
						program["display_duration"] =
							program["duration"] - (program["stop_timestamp"] - to_ts);
					}
					...
					program["on_date"] =
						get_word("week_arr")[start_date.getDay() - 1] +
						" " +
						date +
						"." +
						month +
