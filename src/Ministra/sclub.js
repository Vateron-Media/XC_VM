...
(function () {
	...
	function sclub_constructor() {
		...
		this.init = function () {
			...
			this.subscription_message = new ModalForm({
			...
			this.subscription_message.addItem(
				new ModalFormButton({
					value: get_word("ok_btn"),
					onclick: function () {
		...
		this.hide = function (do_not_reset) {
			...
			this.rent_password_input.on &&
				this.rent_password_input.hide &&
				this.rent_password_input.hide();
		...
		this.init_search_box = function () {
			...
			this.search_box.addCustomEventListener("hide", function () {
				...
				if (
					typeof stb.IsVirtualKeyboardActive == "function" &&
					stb.IsVirtualKeyboardActive()
				) {
		...
		this.fill_short_info = function (item) {
			...
			if (item.rent_info) {
				...
				if (item.rent_info["expires_in"]) {
					info +=
						", " +
						get_word("vclub_rent_expires_in") +
						": " +
						item.rent_info["expires_in"];
				}
				info += "<br>";
			}
		...
		this.handling_block = function (data, row_items, block_name) {
			...
			var active_blocks = [
				"active_today_block",
				"active_yesterday_block",
				"active_week_and_more_block",
			];
			...
			if (data === 1) {
				...
				if (!block_obj.isHidden()) {
					...
					if (passive_blocks.indexOf(block_obj.className) >= 0 && reset_margin) {
						var reset_margin = !passive_blocks
							.splice(0, passive_blocks.indexOf(block_name))
		...
		this.action = function () {
			...
			if (
				this.data_items[this.cur_row].is_movie &&
				this.data_items[this.cur_row].has_files == "0"
			) {
				this.check_for_pass(true);
				return;
			}
				...
				function (result) {
					...
					if (!result) {
						...
						if (!/[^0-9\.,]/.test(this.price_confirm.price)) {
							this.price_confirm.price_str =
								this.price_confirm.price_str +
								get_word("package_price_measurement");
						}

						this.price_confirm.video_id = video_id;
						_debug("this.price_confirm.video_id", this.price_confirm.video_id);

						var msg = get_word("rent_movie_price_text").format(
							this.price_confirm.price_str
						);
						...
						if (result.hasOwnProperty("rent_duration") && result.rent_duration != 0) {
							msg +=
								"<br>" +
								get_word("rent_duration_text").format(result.rent_duration);
						}
		...
		this.check_for_pass = function (play_url, storage) {
			...
			if (this.current_movie.for_rent && !this.current_movie.hasOwnProperty("rent_info")) {
				self.rent_confirm.video_id =
					self.data_items[self.cur_row].video_id || self.data_items[self.cur_row].id;
		...
		this.check_for_series = function (play_url, storage) {
			...
			if (this.data_items[this.cur_row].has_files == "0") {
				if (
					this.data_items[this.cur_row].series &&
					this.data_items[this.cur_row].series.length > 0
				) {
					var self = this;
					...
					this.series_switch.show(
						this.data_items[this.cur_row].series,
						this.data_items[this.cur_row].cur_series
					);
				} else {
		...
		this.play = function (play_url, storage, callback) {
			...
			if (
				this.data_items[this.cur_row].cmd.indexOf("://") < 0 ||
				this.data_items[this.cur_row].protocol == "custom"
			) {
				stb.player.on_create_link = function (result) {
					_debug("sclub.on_create_link", result);
					...
					if (result.error == "limit") {
						...
						if (
							result.hasOwnProperty("cmd") &&
							result.cmd &&
							result.cmd.indexOf("://") != -1 &&
							module.downloads &&
							module.downloads.identical_download_exist(url)
						) {
							...
							if (result.length && result.length >= 2) {
								...
								var callback = function () {
									...
									if (movie.hasOwnProperty("subtitles")) {
										stb.player.cur_media_item.subtitles = movie.subtitles.map(
											function (item, idx) {
								...
								stb.player.cur_media_item.subtitles = result.subtitles.map(
									function (item, idx) {
										item.pid = "external_" + idx;
										item.lang = [item.lang, ""];
										return item;
		...
		this.add_download = function (item, url) {
			...
			if (
				this.data_items[this.cur_row].cur_series &&
				parseInt(this.data_items[this.cur_row].cur_series, 10) != 0
			) {
	...
	sclub.sidebar.init_items("abc", {
		header: word["vclub_by_letter"],
		width: 26,
		align: "center",
	});
	...
	sclub.init_sort_menu(sort_menu, {
		offset_x: 217,
		color: "green",
	});
	...
	sclub.init_view_menu(
		[
			{
				label: word["vclub_list_w_info"],
				cmd: function () {
					this.parent.set_middle_container();
				},
			},
			{
				label: word["vclub_list"],
				cmd: function () {
	...
	sclub.init_other_menu(
		[
			{
				label: word["vclub_search_box"],
				cmd: function () {
					this.parent.search_box_switcher();
				},
			},
			{
				label: word["vclub_query_box"],
				cmd: function () {
	...
	sclub.sidebar.dependency = [
		sclub.sort_menu,
		sclub.search_box,
		sclub.view_menu,
		sclub.other_menu,
	];
	sclub.sort_menu.dependency = [
		sclub.sidebar,
		sclub.search_box,
		sclub.view_menu,
		sclub.other_menu,
	];
	sclub.search_box.dependency = [
		sclub.sidebar,
		sclub.sort_menu,
		sclub.view_menu,
		sclub.other_menu,
	];
	sclub.view_menu.dependency = [
		sclub.sidebar,
		sclub.sort_menu,
		sclub.search_box,
		sclub.other_menu,
	];
	sclub.other_menu.dependency = [
		sclub.sidebar,
		sclub.sort_menu,
		sclub.search_box,
		sclub.view_menu,
