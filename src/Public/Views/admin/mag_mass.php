...
<script>
    (function() {
        ...
        if ($.fn.select2) {
            $('#filter, #show_entries, #m_message_type').select2({ width: '100%' });
            ...
            $('#m_selected_channel').select2({
                width: '100%', placeholder: <?= json_encode($language::get('start_typing') . '...'); ?>,
                ajax: { url: './api', dataType: 'json', delay: 250,
        ...
        $('#parent_password').on('keyup', function() { this.value = this.value.replace(/[^0-9]/g, ''); });
        ...
        var rTable = $('#datatable-mass').DataTable({
            ...
            ajax: { url: './table', data: function(d) { d.id = 'mags'; d.filter = getFilter(); d.reseller = getReseller(); } },
            columns: [
                { data: 'mag_id', className: 'text-center' },
                { data: 'username', visible: false },
                { data: 'mac', className: 'text-center' },
                { data: 'stb_type', visible: false },
            ...
            rowCallback: function(row, data) { if (selected.indexOf(String(data.mag_id)) !== -1) { $(row).addClass('table-active'); } },
        ...
        document.getElementById('mass-form').addEventListener('submit', function(e) {
            ...
            fetch('post.php?action=mag_mass', { method: 'POST', body: fd, headers: { 'X-Requested-With': 'XMLHttpRequest' } })
