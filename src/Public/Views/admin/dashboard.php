			...
			<div class="row">
				<div class="col-12">
					<div class="page-title-box">
						<div class="page-title-right" <?php if ($rMobile): ?>style="width: 100%" <?php endif; ?>>
							<ol class="breadcrumb m-0" style="width: <?php echo $rMobile ? '100%' : '250px'; ?>;">
								<select id="server_id" class="form-control">
									<option <?php if (!RequestManager::has('server_id')) echo 'selected'; ?><?= $language::get('valueall_servers') ?></option>
									<?php foreach ($rServers as $rServerItem): ?>
										<?php if ($rServerItem['enabled']): ?>
											<option value="<?php echo $rServerItem['id']; ?>" <?php if (RequestManager::has('server_id') && RequestManager::get('server_id') == $rServerItem['id']) echo 'selected'; ?>>
						...
						<?php if (!$rMobile): ?>
							<h4 class="page-title">
								<?php if (RequestManager::has('server_id')): ?>
									<?php echo ServerRepository::getAll()[intval(RequestManager::get('server_id'))]['server_name']; ?>&nbsp;
									<a href='server_view?id=<?php echo intval(RequestManager::get('server_id')); ?>' class='btn btn-light waves-effect waves-light btn-xs tooltip-right' title='View Server'><i class='mdi mdi-chart-line'></i></a>
									<a href='process_monitor?server=<?php echo intval(RequestManager::get('server_id')); ?>' class='btn btn-light waves-effect waves-light btn-xs tooltip-right' title='Process Monitor'><i class='mdi mdi-chart-bar'></i></a>
								<?php else: ?>
									Dashboard
								<?php endif; ?>
							</h4>
	...
	if (!$rMobile) {
		if ($rSettings['dashboard_stats']) {
			...
			if (RequestManager::has('server_id')) {
				echo '&server_id=' . intval(RequestManager::get('server_id'));
			}
