...
<form id="package-form" autocomplete="off">
    ...
    <div class="card mb-6">
        <div class="card-header">
            <h5 class="mb-0"><?= $language::get('options'); ?></h5>
        </div>
        <div class="card-body">
            ...
            <div class="row mb-6">
                <div class="col-md-6">
                    ...
                    <select name="force_server_id" id="force_server_id" class="form-select">
                        <option <?= (!$rIsEdit || (int) $rPackage['force_server_id'] === 0) ? 'selected ' : ''; ?><?= $language::get('value0disabled'); ?></option>
    ...
    <div class="card mb-6">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0"><?= $language::get('bouquets'); ?></h5>
            <div class="btn-group btn-group-sm">
...
<script>
    (function() {
        ...
        document.getElementById('package-form').addEventListener('submit', function(e) {
            ...
            btn.disabled = true;
            fetch('post.php?action=package', { method: 'POST', body: new FormData(e.target), headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                .then(function(r) { return r.text(); })
                .then(function(txt) {
                    var dt; try { dt = JSON.parse(txt); } catch (err) { dt = { result: false }; }
                    if (dt && dt.result !== false) { window.location.href = dt.location || 'packages'; return; }
                    btn.disabled = false;
                    alert(errText);
                })
