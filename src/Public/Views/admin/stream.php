            ...
            <div class="tab-content p-0">
                ...
                <div class="tab-pane fade" id="tab-advanced" role="tabpanel">
                    <div class="row g-3 mb-6">
                        <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="gen_timestamps" name="gen_timestamps" value="1" <?= (!$rIsEdit || ($rStream['gen_timestamps'] ?? 0) == 1) ? 'checked' : ''; ?>><label class="form-check-label" for="gen_timestamps">Generate PTS</label></div></div>
                        <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="read_native" name="read_native" value="1" <?= ($rIsEdit && ($rStream['read_native'] ?? 0) == 1) ? 'checked' : ''; ?>><label class="form-check-label" for="read_native">Native Frames</label></div></div>
                        <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="stream_all" name="stream_all" value="1" <?= ($rIsEdit && ($rStream['stream_all'] ?? 0) == 1) ? 'checked' : ''; ?>><label class="form-check-label" for="stream_all">Stream All Codecs</label></div></div>
                        <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="allow_record" name="allow_record" value="1" <?= (!$rIsEdit || ($rStream['allow_record'] ?? 0) == 1) ? 'checked' : ''; ?>><label class="form-check-label" for="allow_record"><?= $language::get('allow_recording'); ?></label></div></div>
                        <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="direct_source" name="direct_source" value="1" <?= ($rIsEdit && ($rStream['direct_source'] ?? 0) == 1) ? 'checked' : ''; ?>><label class="form-check-label" for="direct_source">Direct Source</label></div></div>
                        <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="direct_proxy" name="direct_proxy" value="1" <?= ($rIsEdit && ($rStream['direct_proxy'] ?? 0) == 1) ? 'checked' : ''; ?>><label class="form-check-label" for="direct_proxy">Direct Stream</label></div></div>
                        <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="fps_restart" name="fps_restart" value="1" <?= ($rIsEdit && ($rStream['fps_restart'] ?? 0) == 1) ? 'checked' : ''; ?>><label class="form-check-label" for="fps_restart">Restart on FPS Drop</label></div></div>
                    ...
                    <div class="row g-3 mb-6">
                        <div class="col-md-6"><div class="form-check form-switch"><input class="form-check-input" type="checkbox" id="skip_ffprobe" name="skip_ffprobe" value="1" <?= (isset($rStreamOptions[21]) && $rStreamOptions[21]['value'] == 1) ? 'checked' : ''; ?>><label class="form-check-label" for="skip_ffprobe">Skip FFProbe</label></div></div>
...
<div class="modal fade" id="providerModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title mb-0"><?= $language::get('provider_streams'); ?></h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <div class="card-datatable table-responsive">
                    <table id="datatable-provider-streams" class="table"><thead><tr><th class="text-center"><?= $language::get('icon'); ?></th><th><?= $language::get('stream_name'); ?></th><th><?= $language::get('provider'); ?></th><th class="text-center"><?= $language::get('actions'); ?></th></tr></thead><tbody></tbody></table>
                </div>
            </div>
        </div>
    </div>
...
<script>
    (function() {
        ...
        function renderProbe(html) {
            ...
            if (rows.length < 2) { return '<span class="badge bg-label-danger"><i class="icon-base ti tabler-alert-triangle me-1"></i>Probe failed</span>'; }
            var v = rows.eq(1).find('td').map(function() { return $(this).text().replace(/ /g, ' ').trim(); }).get();
            var contIcon = rows.eq(0).find('td').last().find('i').attr('class') || '';
            var mpegts = contIcon.indexOf('check') !== -1;
            var out = '';
            if (v[0]) { out += probeBadge('secondary', 'tabler-aspect-ratio', v[0]); }
            if (v[1]) { out += probeBadge('primary', 'tabler-video', v[1]); }
            if (v[2]) { out += probeBadge('info', 'tabler-volume', v[2]); }
            if (v[3]) { out += probeBadge('secondary', 'tabler-gauge', v[3]); }
        ...
        $('#transcode_profile_id, #days_to_restart, #on_demand, #llod, #tv_archive_server_id, #vframes_server_id').select2({ width: '100%', dropdownParent: $('#stream-form') });
        function select2Ajax(sel, action, ph) {
            $(sel).select2({
                width: '100%', placeholder: ph, allowClear: true, dropdownParent: $('#tab-advanced'),
                ajax: {
                    url: './api', dataType: 'json', cache: true,
                    data: function(params) { return { search: params.term, action: action, page: params.page }; },
        ...
        if (list) {
            list.addEventListener('click', function(e) {
                ...
                if (btn.classList.contains('btn-src-up') && row.previousElementSibling) { row.parentNode.insertBefore(row, row.previousElementSibling); }
                else if (btn.classList.contains('btn-src-down') && row.nextElementSibling) { row.parentNode.insertBefore(row.nextElementSibling, row); }
                else if (btn.classList.contains('btn-src-remove')) {
                    if (list.querySelectorAll('.source-row').length > 1) { row.remove(); }
                    else { row.querySelector('.src-input').value = ''; row.querySelector('.src-info').innerHTML = ''; }
                }
            });
        }
        window.addStream = function(url) {
            var rows = list.querySelectorAll('.source-row'), last = rows[rows.length - 1];
        ...
        if (addBtn) { addBtn.addEventListener('click', function() { window.addStream(); }); }
        var scanBtn = document.getElementById('scan-sources');
        if (scanBtn) {
            scanBtn.addEventListener('click', function() {
                var onlineSrv = $('#server_tree').jstree(true).get_json('source', { flat: true });
                ...
                list.querySelectorAll('.source-row').forEach(function(row) {
                    ...
                    $.get('./api?action=probe_stream&url=' + encodeURIComponent(url) + '&user_agent=' + encodeURIComponent($('#user_agent').val()) + '&proxy=' + encodeURIComponent($('#http_proxy').val()) + '&cookies=' + encodeURIComponent($('#cookie').val()) + '&headers=' + encodeURIComponent($('#headers').val()) + '&server=' + server, function(data) { info.innerHTML = renderProbe(data); });
                });
            });
        }
        ...
        if (provBtn) {
            var provTable = $('#datatable-provider-streams').DataTable({
                ...
                ajax: { url: './table', data: function(d) { d.id = 'provider_streams'; d.type = 'live'; } },
        ...
        function evaluateServers() {
            var v = $('#vframes_server_id').val(), t = $('#tv_archive_server_id').val(), o = $('#on_demand').val();
            $('#on_demand').empty();
            $('#vframes_server_id').empty().append(new Option('Disabled', 0));
            $('#tv_archive_server_id').empty().append(new Option('Disabled', 0));
            $($('#server_tree').jstree(true).get_json('source', { flat: true })).each(function(i, val) {
                if (val.parent !== '#') {
        ...
        function evaluateDirectSource() {
            ...
            dsFields.forEach(function(id) { setDis(id, ds); });
            setDis('direct_proxy', !ds);
            ['user_agent', 'http_proxy', 'cookie', 'headers'].forEach(function(id) { setDis(id, !(dp || !ds)); });
        }
        ...
        document.getElementById('stream-form').addEventListener('submit', function(e) {
            e.preventDefault();
            if (!isImport && !document.getElementById('stream_display_name').value.trim()) { alert(errText); return; }
            if (isImport && !document.getElementById('m3u_file').value) { alert(errText); return; }
            ...
            btn.disabled = true;
            fetch('post.php?action=stream', { method: 'POST', body: new FormData(e.target), headers: { 'X-Requested-With': 'XMLHttpRequest' } })
                ...
                .then(function(txt) {
                    ...
                    if (dt && dt.result !== false) {
                        ...
                        else { window.location.href = dt.location || 'streams'; }
                        return;
                    }
                    btn.disabled = false; evaluateDirectSource(); alert(errText);
                })
                .catch(function() { btn.disabled = false; evaluateDirectSource(); alert(errText); });
        });
    })();
</script>
</body>
