...
window.PlayerLogin = (function () {
  ...
  const renderSavedAccounts = () => {
      ...
      container.innerHTML = `
        <div class="text-center py-6 px-3">
          ...
          <p class="text-body-secondary small mb-4">
            Accounts you sign into will appear here for fast 1-click access.
          </p>
          <button type="button" class="btn btn-sm btn-outline-primary" id="btn-switch-to-account-tab">
            <i class="icon-base bx bx-user me-1"></i> Sign In With Account
          </button>
        </div>
      `;
    ...
    accounts.forEach((acc) => {
      ...
      if (isExpired) {
        ...
        statusBadge = '<span class="badge bg-label-info rounded-pill"><i class="icon-base bx bx-key me-1"></i>Active Code</span>';
      } else {
        statusBadge = '<span class="badge bg-label-success rounded-pill">Active</span>';
      }
      ...
      html += `
        <div class="saved-account-card d-flex align-items-center justify-content-between p-3 rounded">
          <div class="d-flex align-items-center gap-3 overflow-hidden me-2">
            <div class="saved-account-avatar bg-label-primary text-primary">
              ${escapeHtml(initials)}
            </div>
            <div class="text-truncate">
              <h6 class="mb-0 fw-semibold text-truncate">${escapeHtml(acc.name || acc.username)}</h6>
              <div class="d-flex align-items-center gap-2 mt-1 flex-wrap">
                ${statusBadge}
                ${acc.exp_date_formatted ? `<small class="text-body-secondary text-truncate">${escapeHtml(acc.exp_date_formatted)}</small>` : ''}
              </div>
            </div>
          </div>
  ...
  /**
   * Execute 1-click Quick Login from Tab 3.
   */
  const performQuickLogin = async (btn, username, password, code) => {
   ...
   * show saved accounts or account tab.
   */
